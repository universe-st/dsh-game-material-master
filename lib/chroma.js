    ...
    for (let dr = -2; dr <= 2; dr++) {
        for (let dg = -2; dg <= 2; dg++) {
            for (let db = -2; db <= 2; db++) {
                ...
                if (distSq <= CLUSTER_STEP_SQ + 1e-6)
                    out.push([dr, dg, db]);
            }
        }
    }
...
function buildBackgroundPalette(rgba, width, height, globalTolerance) {
    ...
    if (samples.length === 0)
        return { grid, samples: 0, kept: 0, dropped: 0 };
    // 1) 边框颜色量化计数。
    const cellOf = (offset) => ((rgba[offset] >> 4) << 8) | ((rgba[offset + 1] >> 4) << 4) | (rgba[offset + 2] >> 4);
    const counts = new Uint32Array(PALETTE_CELLS);
    for (const offset of samples)
        counts[cellOf(offset)]++;
    ...
    for (let seed = 0; seed < PALETTE_CELLS; seed++) {
        if (counts[seed] === 0 || cluster[seed] !== -1)
            continue;
        ...
        while (queue.length > 0) {
            ...
            for (const [dr, dg, db] of CLUSTER_NEIGHBORS) {
                ...
                const ng = g + dg;
                const nb = b + db;
                if (nr < 0 || nr >= PALETTE_GRID || ng < 0 || ng >= PALETTE_GRID || nb < 0 || nb >= PALETTE_GRID)
                    continue;
                const next = (nr << 8) | (ng << 4) | nb;
                if (counts[next] === 0 || cluster[next] !== -1)
                    continue;
                cluster[next] = id;
                queue.push(next);
            }
    ...
    let largest = 0;
    for (let i = 1; i < clusterSize.length; i++)
        if (clusterSize[i] > clusterSize[largest])
            largest = i;
...
export function segmentBackgroundDetailed(rgba, width, height, globalTolerance, localTolerance) {
