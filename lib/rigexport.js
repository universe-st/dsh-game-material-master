 ...
 */
export function encodeDragonBonesCurve(control) {
    if (control.length < 4)
        return undefined;
    if (control.length % 3 !== 1)
        return undefined;
 ...
 */
export function buildDragonBonesTimeline(kind, frames, frameRate) {
    ...
    for (let i = 0; i < frames.length; i++) {
        ...
        const entry = { duration, ...spec.valueOf(frames[i]) };
        if (!isLast) {
...
export function buildDragonBonesSkeleton(options) {
    ...
    const slots = (spine?.slots ?? []).map((slot) => ({
        name: slot.name,
        parent: slot.bone,
        displayIndex: 0
    }));
    const attachments = spine?.skins?.[0]?.attachments ?? {};
    const skinSlots = (spine?.slots ?? []).map((slot) => {
    ...
    const PATH_POSITION_MODE = { fixed: 0, percent: 1 };
    const PATH_SPACING_MODE = { length: 0, fixed: 1, percent: 2, proportional: 3 };
    const PATH_ROTATE_MODE = { tangent: 0, chain: 1, chainScale: 2 };
    ...
    for (const [id, value] of Object.entries(options.animations ?? {})) {
        ...
        if (spineDeform !== null && typeof spineDeform === "object") {
            for (const [slotName, bySkin] of Object.entries(spineDeform)) {
                const frames = Array.isArray(bySkin?.default) ? bySkin.default : [];
                if (frames.length === 0)
                    continue;
                ffd.push({
                    name: slotName,
                    ...
                    frame: frames.map((frame, index) => ({
                        ...
                        vertices: Array.isArray(frame.vertices) ? frame.vertices.map((n) => num(n, 0)) : []
                    }))
                });
            }
        }
...
function durationSecondsOf(timelines, fallback) {
    let max = 0;
    for (const kinds of Object.values(timelines?.bones ?? {})) {
 ...
 * 的 `frameX` 约定相同，直接照搬。
 */
export function buildDragonBonesTexture(page, options) {
    return {
        name: options.name,
