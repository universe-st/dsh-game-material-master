 ...
 */
export function partProfile(part, bounds, mirror = false) {
...
/** 内容（非透明）包围盒。 */
export function contentBounds(src, threshold = 8) {
    let minX = src.width;
    let minY = src.height;
    let maxX = -1;
    let maxY = -1;
...
/** 两张轮廓位图的交并比。 */
export function profileOverlap(a, b) {
    let intersection = 0;
    let union = 0;
 ...
 */
export function findDuplicateParts(parts) {
    ...
    for (const record of usable)
        parent.set(record.name, record.name);
    const pairwise = [];
    for (let i = 0; i < usable.length; i++) {
        for (let j = i + 1; j < usable.length; j++) {
            ...
            if (ratio < 1 - ASPECT_TOLERANCE || ratio > 1 + ASPECT_TOLERANCE)
                continue;
            const score = profileOverlap(a.profile, b.profile);
            const mirrored = profileOverlap(a.profile, b.mirrored);
 ...
 */
export function assessParts(options) {
    const issues = [];
    ...
    if (duplicates.length > 0) {
        ...
        if (confirmed.length > 0) {
            ...
            issues.push({
                ...
                message: `有 ${confirmed.length} 组部件**朝向相同、轮廓几乎重合**（共 ${total} 块），基本可以确定是同一个部位被画了多遍：` +
                ...
                suggestion: "每组保留 1 块、隐藏其余（卡片上有「隐藏」）。注意判据：**同向**重合才是重复；" +
        ...
        if (uncertain.length > 0) {
            issues.push({
                ...
                message: `有 ${uncertain.length} 组部件轮廓高度重合、但**互为镜像的程度也一样高**（形状本身接近对称），` +
    ...
    if (matches.length > 0) {
        const scores = matches.map((item) => item.score);
        const mean = scores.reduce((sum, value) => sum + value, 0) / scores.length;
        const low = matches.filter((item) => item.score < LOW_SCORE);
        const garbage = matches.filter((item) => item.score < GARBAGE_SCORE);
        if (mean < LOW_SCORE) {
            issues.push({
                level: "error",
                code: "layout-low-confidence",
                parts: low.map((item) => item.name),
                message: `装配平均相似度只有 ${mean.toFixed(3)}（正常应 > ${LOW_SCORE}），${low.length}/${matches.length} 个部件低于阈值` +
                ...
                suggestion: "用 read_image 看参考图与 parts-montage.png，判断每块部件在参考图的哪个位置，用 " +
    ...
    if (typeof options.expectedParts === "number" && options.expectedParts > 0 && partCount > options.expectedParts) {
        issues.push({
            ...
            code: "too-many-parts",
            parts: [],
            message: `拆出 ${partCount} 个部件，但参考图里大致只能看到 ${options.expectedParts} 个独立部位——` +
    ...
    if (errors.some((issue) => issue.code === "too-many-parts"))
        score -= 0.25;
    if (matches.length > 0) {
