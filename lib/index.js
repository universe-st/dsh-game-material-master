...
import { applyTurnPicks, clearVideos, disposePipeline, ensurePoller, listJobs, pollVideosOnce, resetTurnPicks, setImageMode, setTurnPick, setTurnPicks, startAllImages, startCompose, startExtract, startImage, startRekey, startTurnFrames, startTurnVideo, startVideos } from "./pipeline.js";
...
export class GameStudioGateway extends TypertRemoteService {
    ...
    */
    ...
    async setRigConstraints(payload) {
        ...
        const patches = raw.map((item) => {
            const record = asRecord(item);
            const patch = { name: asString(record.name), bone: asString(record.bone) };
            if (typeof record.target === "string")
                patch.target = record.target;
            if (typeof record.chain === "number")
                patch.chain = record.chain;
            if (typeof record.bendPositive === "boolean")
                patch.bendPositive = record.bendPositive;
            if (typeof record.weight === "number")
                patch.weight = record.weight;
            return patch;
        });
    ...
    async setRigMesh(payload) {
        ...
        const patches = raw.map((item) => {
            const record = asRecord(item);
            const patch = { name: asString(record.name) };
            if (typeof record.cols === "number")
                patch.cols = record.cols;
            if (typeof record.rows === "number")
                patch.rows = record.rows;
            if (record.deform === null)
                patch.deform = null;
            else if (record.deform !== undefined) {
                const deform = asRecord(record.deform);
                const parsed = {};
                if (typeof deform.amplitude === "number")
                    parsed.amplitude = deform.amplitude;
                if (typeof deform.cycles === "number")
                    parsed.cycles = deform.cycles;
                if (typeof deform.direction === "number")
                    parsed.direction = deform.direction;
                if (deform.anchor === "top" || deform.anchor === "bottom" || deform.anchor === "none")
                    parsed.anchor = deform.anchor;
                if (typeof deform.duration === "number")
                    parsed.duration = deform.duration;
                patch.deform = parsed;
            }
            return patch;
        });
    ...
    async setRigPath(payload) {
        ...
        const patches = raw.map((item) => {
            ...
            if (Array.isArray(record.points))
                patch.points = record.points.filter((n) => typeof n === "number");
            if (typeof record.closed === "boolean")
                patch.closed = record.closed;
            if (Array.isArray(record.bones))
                patch.bones = record.bones.filter((n) => typeof n === "string");
            if (typeof record.spacing === "number")
                patch.spacing = record.spacing;
            if (typeof record.translateMix === "number")
                patch.translateMix = record.translateMix;
            if (typeof record.rotateMix === "number")
                patch.rotateMix = record.rotateMix;
            return patch;
        });
