 ...
 *
 * `cols/rows` 是网格密度（规则三角化，确定性重算，不落盘）；`deform` 是波形变形，
 * 传 `null` 表示只保留网格、不加变形。
 */
const setRigMeshSchema = z.object({
    jobId: z.string(),
    meshes: z.array(z.object({
        ...
        deform: z
            .object({
            amplitude: z.number().min(0).max(400).optional(),
            cycles: z.number().min(0.25).max(8).optional(),
            direction: z.number().optional(),
            anchor: z.enum(["top", "bottom", "none"]).optional(),
...
/**
 * AI 逐部件重绘（**花钱**：一次 Seedream 调用）。
 *
 * 一个部件进、一个部件出——所以 reskin-app 那套「整张图重绘后再用 SAM /
 * 连通域反向切回各 slot」完全不需要（见方案 §3.1 的范围裁剪）。
 * 结果落成新版本，不满意随时切回上一版。
 */
const runRigRedrawSchema = z.object({
    jobId: z.string(),
    name: z.string(),
    /** 想怎么改（例如「换成深蓝色」「加上金属高光」）。 */
    prompt: z.string(),
    /** 边缘腐蚀半径（像素），用来削掉 AI 生成部件常见的白晕。默认 1。 */
    erode: z.number().optional()
});
...
export const METHODS = [
    ...
    { method: "runRigRedraw", payload: runRigRedrawSchema, result: startedSchema },
...
export const MANIFEST = {
    ...
    invocations: METHODS.map((spec) => ({
        ...
        parameters: spec.payload === undefined
            ? []
            : [
                {
                    name: "payload",
                    wire: "payload",
                    source: "json",
                    codec: { mode: "strict", typeSymbol: `${PACKAGE_NAME}#${spec.method}Payload`, schema: spec.payload }
                }
            ],
        result: { mode: "strict", typeSymbol: `${PACKAGE_NAME}#${spec.method}Result`, schema: spec.result }
    })),
    model: { services: [], events: [], objects: [] }
};
/** 客户端只需要方法名与是否有 payload；这里导出给浏览器半区复用同一张清单。 */
export const METHOD_NAMES = METHODS.map((spec) => ({
    method: spec.method,
 ...
 */
export const TOOL_METHODS = METHODS.filter((spec) => spec.clientOnly !== true).map((spec) => ({ method: spec.method, payload: spec.payload !== undefined }));
