...
import { DEFAULT_DRAW_ORDER, RIG_ANIMATIONS, RIG_GRID_COLUMNS, RIG_GRID_ROWS, buildAtlasText, buildSkeleton, defaultAnimationIds, defaultPartNames, drawRankOf, isAnimationId, packAtlas, partLabel } from "./spine.js";
import { buildPreviewHtml } from "./rigpreview.js";
import { validateAtlas, validateSkeletonAtlasMatch, validateSpineWire } from "./rigvalidate.js";
...
export async function buildRigOutput(jobId) {
    ...
    try {
        ...
        if (!wireReport.ok) {
            throw new Error(`skeleton.json 未通过 Spine 4.2 校验（${wireReport.errors.length} 处）：` +
