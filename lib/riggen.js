...
import { alphaBounds, blitRgba, boxMask, compositeParts, createRgba, detectBackground, groupComponentsByProximity, resizeRgba, rotateRgba, segmentWithLabels, sideBySide, solveLayout, tintRgba } from "./rigpose.js";
...
import { defaultSemanticsOf, mergeSemantics, validateSemantics } from "./rigsemantics.js";
...
export async function uploadRigTexture(jobId, name, base64, options = {}) {
    const job = await readRigJob(jobId);
    if (job === undefined)
        throw new Error(`任务不存在：${jobId}`);
