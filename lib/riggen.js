...
import { boxMask, compositeParts, createRgba, detectBackground, resizeRgba, segmentComponents, sideBySide, solveLayout } from "./rigpose.js";
...
export async function segmentSheet(jobId) {
    ...
    for (let index = 0; index < columns * rows; index++) {
        ...
        for (let i = 0; i < trimmed.width * trimmed.height; i++)
            if (trimmed.data[i * 4 + 3] > 128)
                opaque++;
        if (list.length > 1)
            extraFragments += list.length - 1;
        next.push({
            name,
