...
import { DEFAULT_DRAW_ORDER, RIG_ANIMATIONS, RIG_GRID_COLUMNS, RIG_GRID_ROWS, animationDurationOf, animationPresetOf, buildAtlasText, buildSkeleton, defaultAnimationIds, defaultPartNames, drawRankOf, isAnimationId, normalizeAnimationSettings, packAtlas, partLabel } from "./spine.js";
...
function invalidateFrom(job, stage) {
    ...
    if (from < STAGE_ORDER.indexOf("rig"))
        job.rig = { status: "empty" };
    else
        job.rig.approved = false;
    if (from < STAGE_ORDER.indexOf("atlas"))
        job.atlas = { status: "empty" };
    else
        job.atlas.approved = false;
}
// ── 后台任务 ────────────────────────────────────────────────────────────
