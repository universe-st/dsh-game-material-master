...
import { blitRgba, boxMask, compositeParts, createRgba, detectBackground, groupComponentsByProximity, resizeRgba, rotateRgba, segmentWithLabels, sideBySide, solveLayout } from "./rigpose.js";
...
export async function buildAtlas(jobId) {
    ...
    try {
        ...
        await writeFile(rigAssetPath(jobId, "atlas/skeleton.atlas"), buildAtlasText("skeleton.png", packed.width, packed.height, packed.placements), "utf8");
        const fresh = await readRigJob(jobId);
        if (fresh === undefined)
            return;
        fresh.atlas = {
