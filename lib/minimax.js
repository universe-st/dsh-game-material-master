...
function assertKey(apiKey) {
    if (apiKey.trim() === "")
        throw new MiniMaxError("尚未配置 MiniMax API Key（设置 → 八方向图工坊）");
}
/**
 * 解析错误响应。平台上有两种风格：
...
export async function queryVideo(input) {
    ...
    if (protocolOf(input.model) === "v2") {
        const payload = await requestJson(`${root}/v2/query/video_generation/${encodeURIComponent(input.taskId)}`, { method: "GET", headers: headers(input.apiKey) }, input.timeoutMs, "查询视频任务");
    ...
    const payload = await requestJson(`${root}/v1/query/video_generation?task_id=${encodeURIComponent(input.taskId)}`, { method: "GET", headers: headers(input.apiKey) }, input.timeoutMs, "查询视频任务");
...
export async function retrieveFile(input) {
    assertKey(input.apiKey);
    const payload = await requestJson(`${rootOf(input.baseUrl)}/v1/files/retrieve?file_id=${encodeURIComponent(input.fileId)}`, { method: "GET", headers: headers(input.apiKey) }, input.timeoutMs, "获取视频下载地址");
