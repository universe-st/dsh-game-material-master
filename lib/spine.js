...
function meshAttachmentOf(name, part, attachment, spec, context) {
 ...
 */
function buildWeightedVertices(mesh, part, attachment, context) {
    const owned = context.worldOf(part.name);
    if (owned === undefined)
        return undefined;
    if (context.imageBones.length === 0)
        return undefined;
    if (!mesh.weights.some((list) => list.length > 0))
        return undefined;
    ...
    for (let i = 0; i < mesh.vertices.length / 2; i++) {
        ...
        for (const entry of usable) {
            ...
            out.push(context.boneIndex.get(boneName), round(dx * c - dy * s, 3), round(dx * s + dy * c, 3), 
...
export function buildSkeleton(options) {
    ...
    for (const [meshName, spec] of Object.entries(options.meshes ?? {})) {
        ...
        if (part === undefined || region === undefined)
            continue;
