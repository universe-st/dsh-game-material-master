...
export function buildAnimations(boneNames, ids) {
...
export function buildSkeleton(options) {
    ...
    const resolveParent = (name, seen) => {
        // ① 语义层显式指定的父级优先。它可能指向一个被隐藏/未生成的部件——
        //    那种情况下不能让骨架断链，所以继续走 ②。
        const explicit = byName.get(name)?.parent;
        let parent = explicit !== undefined && explicit !== name && boneNames.has(explicit) ? explicit : undefined;
        ...
        if (parent === undefined && name !== "root") {
            ...
            if (byName.has("torso") && name !== "torso")
                parent = "torso";
            else if (byName.has("hip") && name !== "hip")
                parent = "hip";
        }
        return parent;
    };
    ...
    const animations = buildAnimations(bones.map((bone) => bone.name), options.animationIds);
 ...
 */
export function packAtlas(sizes, padding = 2) {
    if (sizes.length === 0)
...
export function buildAtlasText(pageName, width, height, placements) {
    const lines = [pageName, `size: ${width},${height}`, "format: RGBA8888", "filter: Linear,Linear", "repeat: none"];
    for (const item of placements) {
        lines.push(item.name, "  rotate: false", `  xy: ${item.x}, ${item.y}`, `  size: ${item.width}, ${item.height}`, `  orig: ${item.width}, ${item.height}`, "  offset: 0, 0", "  index: -1");
    }
    return `${lines.join("\n")}\n`;
}
