...
function intakeFor(module, id, told, config, target) {
    ...
    if (module === "sprite") {
        ...
        ask("generateMode", "阶段①「八方向绿幕图」用哪种生成方式？默认是「转圈截帧」（先生成一段原地匀速转一整圈的绿幕视频，再按时间截出八个方向——八个方向同源、一致性最好，一次视频调用；位置可以在时间轴上拖）；也可以选「逐方向生图」（每个方向单独生图，单张更清晰，但八方向容易不一致，八次调用）。", "这一步决定后面整条链路的产物形态与花费，先定下来再动手。", "转圈截帧：setImageMode({projectId,mode:'turn'}) → runTurnVideo({projectId}) → game_material_wait（视频到手会自动抽帧切图）→ 需要时 setTurnPick({projectId,key,index}) 调位置；逐方向生图：setImageMode({projectId,mode:'direct'}) → runImages({projectId})。");
...
function renderRigStageDetail(stage) {
    ...
    if (stage.stage === "parts") {
        ...
        for (const entry of stage.dragonBones ?? [])
            push("贴图描述（DragonBones）", entry.url);
...
function renderStatus(value) {
    if (Array.isArray(value.projects)) {
        const lines = [
            ...
            ...value.projects.map((p) => `  - ${p.id} ${p.name} — 图 ${p.imageReady}/8 · 视频 ${p.videoReady}/8 · 帧 ${p.framesReady}/8${p.sheetReady ? " · 整图✓" : ""}${p.imageMode === "turn" ? " · 阶段①转圈截帧" : p.imageMode === "direct" ? " · 阶段①逐方向生图" : ""}`),
    ...
    if (value.imageMode !== undefined && value.turn !== undefined) {
        ...
        if (value.imageMode === "turn") {
            lines.push(`  转圈视频：${turn.videoStatus ?? "empty"}${turn.remoteStatus ? `（${turn.remoteStatus}）` : ""}` +
                `${turn.frameCount > 0 ? ` · 候选帧 ${turn.frameCount} 张 / 时长 ${Number(turn.duration ?? 0).toFixed(2)} 秒` : " · 还没有候选帧"}` +
                `${turn.stale === true ? " · 候选帧已过期，需要重新抽帧" : ""}`);
            if (typeof turn.videoUrl === "string")
                lines.push(`  转圈视频地址：${turn.videoUrl}`);
            if (typeof turn.stripUrl === "string")
                lines.push(`  整圈缩略条带：${turn.stripUrl}`);
            const picks = value.turnPicks ?? {};
            const pickKeys = Object.keys(picks);
            if (pickKeys.length > 0)
                lines.push(`  八个截帧位置：${pickKeys.map((key) => `${key}=${picks[key]}`).join(" ")}`);
            if (typeof turn.error === "string" && turn.error !== "")
                lines.push(`  转圈视频错误：${turn.error}`);
        }
    }
...
function renderRigQa(qa) {
    ...
    if (qa.expectedParts !== undefined)
        lines.push(`      （参考图目测约 ${qa.expectedParts} 个独立部位）`);
    return lines.join("\n");
}
    ...
    if (images !== undefined && images.ready < images.total) {
        ...
        if (snapshot.imageMode === "turn") {
            if (turn.videoStatus !== "ready") {
                ...
                actions.push(`转圈视频已就绪，还差 ${images.total - images.ready} 张方向图：game_material_call({method:'runTurnFrames', payload:{projectId}}) 然后 game_material_wait（本地免费）`);
            }
            if (turn.frameCount > 0) {
                actions.push(`★ 先核对八个朝向：read_image 看 turn.stripUrl（整圈候选帧条带），确认每个方向落在第几帧——左侧脸/右侧脸反了就是转圈方向反了，` +
...
function rigNextActions(snapshot, stage) {
    ...
    if (qa !== null && qa !== undefined && qa.ok !== true) {
        const duplicate = (qa.issues ?? []).find((issue) => issue.code === "duplicate-parts");
        const lowConfidence = (qa.issues ?? []).find((issue) => issue.code === "layout-low-confidence");
        if (duplicate !== undefined) {
            actions.push("**拆件质检不通过：有部件被重复画了多遍**。先用 read_image 看 partsMontagePath 确认哪几块是同一部位，" +
                "用 setRigPartVisibility({jobId,name,hidden:true}) 隐藏多余的（每组留 1 块），再重跑 runRigQa 与 runRigLayout");
        }
        if (lowConfidence !== undefined) {
            actions.push("**装配相似度整体偏低**：模板匹配对「重新绘制过、与参考图不像」的部件失手。" +
