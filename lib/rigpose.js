 ...
 * 位置关系就稳定了。
 */
export function padToSquare(src, background = [255, 255, 255]) {
 ...
 */
export function maskAndFit(src, mask, crop, targetWidth, targetHeight) {
    // ⓪ 先把模型输出对齐到补边正方形的尺寸。
    ...
    for (let y = 0; y < mask.height; y++) {
        for (let x = 0; x < mask.width; x++) {
            ...
            if (sx < 0 || sy < 0 || sx >= square.width || sy >= square.height)
                continue;
 ...
 */
export function erodeAlpha(src, radius) {
    if (radius <= 0)
 ...
 * 有这组数字就能在落盘前拦住，并说清原因。
 */
export function imageStats(src, alphaThreshold = 128) {
    let opaque = 0;
    let sum = 0;
    let sumSquares = 0;
    for (let i = 0; i < src.width * src.height; i++) {
        const at = i * 4;
        if (src.data[at + 3] < alphaThreshold)
            continue;
    ...
    if (opaque === 0)
        return { opaque: 0, meanLuma: 0, stdDev: 0 };
