...
`;
        ...
        function SequenceModule(props) {
            ...
            return h(React.Fragment, null, h("div", { className: "SPR_body" }, h(JobSidebar, {
                ...
                : h(React.Fragment, null, h("div", { className: "SPR_card" }, h("div", { className: "SPR_cardHead" }, h("h3", null, "① 视频输入")), h("p", { className: "SPR_hint" }, "平台规定两种模式互斥：首尾帧模式以一张图作为起始画面；多模态参考模式用参考图+参考视频来约束风格与动作。"), h("div", { className: "SPR_toolbar" }, h("select", {
                    ...
                    ? h(React.Fragment, null, h("span", { className: "SPR_fieldLabel" }, "首帧图（必填）"), h(RefRow, { job, ref: job.refs.firstFrame, kind: "firstFrame", api, reload: load, setNotice }), h("span", { className: "SPR_fieldLabel" }, "尾帧图（选填）"), h(RefRow, { job, ref: job.refs.lastFrame, kind: "lastFrame", api, reload: load, setNotice }), h(UploadBox, { label: "把首帧图拖到这里", accept: "image/*", busy: uploading, onFiles: (files) => void uploadRef(files, "firstFrame") }))
                    ...
                    onBlur: () => { if (suffixDraft !== (job.suffix ?? ""))
                        void saveJob({ suffix: suffixDraft }); }
                }), h("div", { className: "SPR_fields" }, h("label", { className: "SPR_field" }, h("span", { className: "SPR_fieldLabel" }, "视频模型"), h("input", { className: "SPR_input", value: settingsDraft.model ?? "", onChange: (event) => setSettingsDraft({ ...settingsDraft, model: event.target.value }), onBlur: () => void saveJob({ settings: settingsDraft }) })), h(NumField, { label: "时长（秒）", value: settingsDraft.duration ?? 5, min: 1, max: 30, onChange: (v) => { setSettingsDraft({ ...settingsDraft, duration: v }); void saveJob({ settings: { ...settingsDraft, duration: v } }); } }), h("label", { className: "SPR_field" }, h("span", { className: "SPR_fieldLabel" }, "分辨率"), h("select", { className: "SPR_input", value: settingsDraft.resolution ?? "768P", onChange: (event) => { setSettingsDraft({ ...settingsDraft, resolution: event.target.value }); void saveJob({ settings: { ...settingsDraft, resolution: event.target.value } }); } }, h("option", { value: "768P" }, "768P"), h("option", { value: "2K" }, "2K"), h("option", { value: "1080P" }, "1080P"), h("option", { value: "480P" }, "480P")))), h("div", { className: "SPR_toolbar" }, h(Btn, {
                    primary: true,
                    disabled: promptDraft.trim() === "",
        ...
        function RefRow(props) {
            ...
            return h("div", { className: "SPR_toolbar" }, h("img", { className: "SPR_thumbSm", src: `${job.assetBase}${ref.file}?v=${job.updatedAt}`, alt: ref.name }), h("span", { className: "SPR_refRow" }, ref.name), h(Btn, {
                danger: true,
                onClick: async () => {
                    try {
        ...
        function ConfigSection(props) {
            ...
            return h("section", { className: "SPR_settings" }, h("h2", { style: { margin: 0, fontSize: 15 } }, "八方向图工坊"), h("p", { className: "SPR_hint" }, "配置两家模型的 API Key 与整条流水线的默认参数。Key 只保存在本机 DSH 数据目录下的 game-material-master/config.json（真实路径见文末「数据位置」），界面里始终脱敏显示。"), notice !== null
            ...
            }, minimaxCaps.resolutions.map((value) => h("option", { key: value, value }, value)))), h("label", { className: "SPR_field" }, h("span", { className: "SPR_fieldLabel" }, "Base URL（主机根，不含 /v1）"), h("input", {
                className: "SPR_input",
                list: "SPR_minimax_hosts",
                value: config.minimaxBaseUrl,
                onChange: (event) => void patch({ minimaxBaseUrl: event.target.value })
            }), h("datalist", { id: "SPR_minimax_hosts" }, (config.minimaxHosts ?? []).map((host) => h("option", { key: host.id, value: host.id }, host.label))), h("span", { className: "SPR_fieldLabel" }, `当前协议：${minimaxCaps.protocol === "v2" ? "v2（/v2/video_generation）" : "v1（/v1/video_generation）"}`))), h("div", { className: "SPR_toolbar" }, h(Btn, {
