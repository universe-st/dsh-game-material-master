        ...
        function RigAssemblyEditor({ job, api, run, busy, activeKey }) {
            ...
            const commit = (patches, label) => {
                ...
                for (const patch of patches)
                    snapshot[patch.name] = items[patch.name] ?? null;
            ...
            // ── 工具条 ────────────────────────────────────────────────
            h("div", { className: "SPR_asmBar" }, h("span", { className: "SPR_asmGroup" }, h("span", { className: "SPR_fieldLabel" }, "缩放"), h(Btn, { onClick: () => setZoom((z) => Math.max(0.25, z - 0.25)), title: "缩小" }, "−"), h(Btn, { onClick: () => setZoom(1) }, `${Math.round(zoom * 100)}%`), h(Btn, { onClick: () => setZoom((z) => Math.min(4, z + 0.25)), title: "放大" }, "+")), h("span", { className: "SPR_asmGroup" }, h(Btn, { on: showReference, onClick: () => setShowReference((v) => !v), title: "把参考图叠在下面，方便对位" }, "参考图底图"), h(Btn, { on: showBoxes, onClick: () => setShowBoxes((v) => !v) }, "显示边框"), h(Btn, { on: snapEnabled, onClick: () => setSnapEnabled((v) => !v), title: "拖动时吸附到其它部件的边与中线" }, "吸附"), h(Btn, { on: lockRatio, onClick: () => setLockRatio((v) => !v), title: "拖角手柄时保持宽高比" }, "锁等比")), h("span", { className: "SPR_asmGroup" }, h(Btn, { onClick: undo, disabled: history.length === 0 }, `撤销${history.length > 0 ? `(${history.length})` : ""}`), h(Btn, { onClick: redo, disabled: future.length === 0 }, "重做")), h("span", { className: "SPR_spacer" }), h("span", { className: "SPR_asmHint" }, `已放置 ${placedNames.length}/${(job.parts ?? []).filter((part) => part.status === "ready").length}　·　拖部件移动、拖角缩放、方向键微调 1px（Shift 10px）、Delete 收回`)), h("div", { className: "SPR_asmBody" }, 
            // ── 画布 ──────────────────────────────────────────────
            h("div", { className: "SPR_asmStage", ref: stageRef }, h("div", {
                ...
                : h(React.Fragment, null, h("div", { className: "SPR_toolbar", style: { flexWrap: "wrap" } }, h("span", { className: "SPR_refRow" }, "任务"), h("select", { className: "SPR_input", style: { width: 260 }, value: jobId ?? "", onChange: (event) => setJobId(event.target.value) }, jobs.map((entry) => h("option", { key: entry.id, value: entry.id }, `${entry.name}（部件 ${entry.partCount}）`))), job !== null ? h("span", { className: "SPR_badge" }, job.id) : null, stageBusy ? h(BusyBadge, { show: true, text: "本阶段进行中…" }) : null), job === null
                    ...
                    stage === "rig"
                        ? h("div", { className: "SPR_rigStage" }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, "③ 骨骼与动画"), h("span", { className: "SPR_rigStageHint" }, "按部件语义自动推骨骼层级，并生成六个动画预设。**本地计算，免费**。下面直接播放验收。"), h(StatusChip, { node: rigStageOf(job, "rig") })), h("div", { className: "SPR_rigEditorRow" }, h(BusyBtn, {
                            ...
                            : h("p", { className: "SPR_hint", style: { marginTop: 10 } }, job.layout?.status === "ready" ? "点「生成骨骼与动画」得到 skeleton.json 与可播放预览。" : "先完成第②步装配定位。"), h(RigAnimationPanel, { job, api, run, busy, activeKey: K_RIG_BONES }), h(RigBoneEditor, { job, api, run, busy, activeKey: K_RIG_BONES }), h("div", { className: "SPR_toolbar", style: { marginTop: 12 } }, h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: true, stage: "rig" }), "第③步已通过"), on: rigStageOf(job, "rig").approved === true, disabled: job.rig?.status !== "ready" }, rigStageOf(job, "rig").approved === true ? "第③步：已通过" : "第③步：通过"), h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: false, stage: "rig" })) }, "取消通过"), h(Btn, { onClick: () => setStage("atlas") }, "下一步：图集 →")))
                        : null, 
                    // ── ④ 图集 ────────────────────────────────────────────
                    stage === "atlas"
                        ...
                        (job.atlas?.pages ?? []).length > 1
                            ...
                            : null, h("div", { className: "SPR_toolbar", style: { marginTop: 12 } }, h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: true, stage: "atlas" }), "第④步已通过"), on: rigStageOf(job, "atlas").approved === true, disabled: job.atlas?.status !== "ready" }, rigStageOf(job, "atlas").approved === true ? "第④步：已通过" : "第④步：通过"), h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: false, stage: "atlas" })) }, "取消通过")), h("p", { className: "SPR_hint", style: { marginTop: 10 } }, "导入 Spine：把 skeleton.json、skeleton.atlas、skeleton.png 三个文件放在同一目录，打开 Spine 时选 skeleton.json 即可。"))
