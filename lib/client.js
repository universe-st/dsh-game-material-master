        ...
        function RigModule(props) {
            ...
            return h("div", { className: "SPR_rigPanel" }, h("div", { className: "SPR_toolbar" }, h("span", { className: "SPR_refRow" }, "生图模型"), h("span", { className: "SPR_badge" }, globalConfig?.arkModel ?? job?.settings?.model ?? "未配置"), h("span", { className: "SPR_refRow" }, "（模型在「设置 → 游戏素材大师」里改）"), h("span", { className: "SPR_spacer" }), headerButtons), notice !== null
                ...
                : h(React.Fragment, null, h("div", { className: "SPR_toolbar", style: { flexWrap: "wrap" } }, h("span", { className: "SPR_refRow" }, "任务"), h("select", { className: "SPR_input", style: { width: 260 }, value: jobId ?? "", onChange: (event) => setJobId(event.target.value) }, jobs.map((entry) => h("option", { key: entry.id, value: entry.id }, `${entry.name}（部件 ${entry.partCount}）`))), job !== null ? h("span", { className: "SPR_badge" }, job.id) : null, stageBusy ? h(BusyBadge, { show: true, text: "本阶段进行中…" }) : null), job === null
                    ? null
                    : h(React.Fragment, null, h(ReviewModeBar, { api, module: "rig", id: job.id, mode: job.reviewMode, onChanged: () => void load(job.id) }), 
                    // ── 第 0 步：角色参考图 ────────────────────────────────
                    ...
                    stage === "rig"
                        ? h("div", { className: "SPR_rigStage" }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, "③ 骨骼与动画"), h("span", { className: "SPR_rigStageHint" }, "按部件语义自动推骨骼层级，并生成六个动画预设。**本地计算，免费**。下面直接播放验收。"), h(StatusChip, { node: rigStageOf(job, "rig") })), h("div", { className: "SPR_rigEditorRow" }, h(BusyBtn, {
                            ...
                            ? h(React.Fragment, null, h("div", { className: "SPR_rigEditorRow" }, h("span", { className: "SPR_refRow" }, "播放动画"), (job.rig.animations ?? []).map((name) => h(Btn, { key: name, on: anim === name, onClick: () => setAnim(name) }, name)), h("span", { className: "SPR_hint" }, "预览里也能拖时间轴、开骨骼网格")), h("iframe", { className: "SPR_rigPreview", src: `${job.rig.preview}#${anim}`, title: "骨骼动画预览" }))
                            : h("p", { className: "SPR_hint", style: { marginTop: 10 } }, job.layout?.status === "ready" ? "点「生成骨骼与动画」得到 skeleton.json 与可播放预览。" : "先完成第②步装配定位。"), h(RigAnimationPanel, { job, api, run, busy, activeKey: K_RIG_BONES }), h(RigBoneEditor, { job, api, run, busy, activeKey: K_RIG_BONES }), 
