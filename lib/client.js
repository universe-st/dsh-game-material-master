...
`;
        ...
        function LoadingOverlay({ show, text, sub }) {
            if (show !== true)
                return null;
            return h("div", { className: "SPR_ovl", role: "status", "aria-live": "polite", "aria-busy": "true" }, h("span", { className: "SPR_spin" }), h("span", { className: "SPR_ovlText" }, text ?? "处理中…"), sub === undefined || sub === null ? null : h("span", { className: "SPR_ovlSub" }, sub));
        }
        /**
        ...
        function MediaBox({ overlay, text, className, children }) {
            return h("div", { className: cls("SPR_thumbBox", className) }, h("div", { style: { visibility: overlay === true ? "hidden" : undefined } }, children), h(LoadingOverlay, { show: overlay === true, text: text ?? "正在生成…" }));
        }
        /** 按钮级别的「正在跑」：转圈 + 文案，并把按钮本身置灰防重复点击。 */
        function BusyBtn({ busy, busyText, children, onClick, ...rest }) {
            return h(Btn, { ...rest, onClick, disabled: busy === true || rest.disabled === true, busy: busy === true }, busy === true ? h("span", { className: "SPR_spinSm" }) : null, busy === true ? busyText ?? "处理中…" : children);
        }
        /** 工具栏上的状态徽章：有任务在跑时显示，同时兼作按钮区的位置占位。 */
        function BusyBadge({ show, text }) {
            if (show !== true)
                return null;
            return h("span", { className: "SPR_busyBadge", role: "status", "aria-live": "polite" }, h("span", { className: "SPR_spinSm" }), text ?? "正在处理…");
        }
        // ── 验收：审核模式与「通过」───────────────────────────────────────────
        /**
        ...
        function ReviewModeBar({ api, module, id, mode, onChanged }) {
            ...
            return h("div", { className: "SPR_toolbar" }, h("span", { className: "SPR_refRow" }, "审核模式"), h("select", {
                className: "SPR_input",
                style: { width: 210 },
                value: mode ?? "",
                disabled: busy || id === null || id === undefined,
                onChange: (event) => void change(event.target.value)
            }, h("option", { value: "" }, "未设置（对话里会先问）"), h("option", { value: "auto" }, "自动审核"), h("option", { value: "manual" }, "每一步人工审核")), h("span", { className: "SPR_refRow" }, mode === "manual"
        ...
        function ApproveBtn(props) {
            ...
            return h(Btn, { on: approved === true, disabled: disabled === true, onClick: onToggle }, approved === true ? "已通过" : idleText ?? "通过");
        }
        // ── 侧栏图标 ─────────────────────────────────────────────────────────
        ...
        function StudioPanel(props) {
            ...
            return h("div", { className: "SPR_root" }, h("div", { className: "SPR_head" }, h("h2", null, "游戏素材大师"), h("span", { className: "SPR_headSub" }, "火山方舟 Seedream 生图 · MiniMax 图生视频 · 本地抠绿幕合成"), h("span", { className: "SPR_spacer" }), module === "sprite"
            ...
            }, h("span", { className: "SPR_moduleTitleRow" }, h("span", { className: "SPR_moduleTitle" }, entry.title), entry.experimental === true ? h("span", { className: "SPR_expTag" }, "实验性") : null), h("span", { className: "SPR_moduleHint" }, entry.hint)))), module === "sprite"
                ? h("div", { className: "SPR_body" }, h("div", { className: "SPR_side" }, h("div", { className: "SPR_sideTitle" }, `项目（${projects.length}）`), projects.length === 0
                    ...
                    h("p", { className: "SPR_hint" }, stageHint(project, stage, activeStage)), stage === "images"
                        ...
                        : null, stage === "preview" ? h(WalkPreview, { project, tasks }) : null), h("div", { className: "SPR_card" }, h("div", { className: "SPR_cardHead" }, h("h3", null, "源图")), h("p", { className: "SPR_hint" }, "上传角色的原始设定图。第一步的正面绿幕图会以它为唯一参考。"), h("div", { className: "SPR_sourceRow" }, project.source !== null
                        ? h("img", {
        ...
        function ImageModeBar({ project, api, start, mode }) {
            return h("div", { className: "SPR_modeBar" }, h("span", { className: "SPR_modeBarLabel" }, "生成方式"), IMAGE_MODES.map((entry) => h("button", {
                key: entry.key,
                type: "button",
                className: "SPR_mode",
            ...
            }, h("span", { className: "SPR_modeTitle" }, entry.title), entry.key === "turn" ? h("span", { className: "SPR_modeTag" }, "默认") : null)), h("span", { className: "SPR_modeBarHint" }, (IMAGE_MODES.find((entry) => entry.key === mode) ?? IMAGE_MODES[0]).hint));
        }
        /**
         * 八个截帧位置的时间轴。
         *
        ...
        function TurnAxis({ project, total, duration, picks, images, api, start, busy }) {
            ...
            const commit = (key, index) => {
                ...
                if (index === current)
                    return;
                void start(() => api.setTurnPick({ projectId: project.id, key, index }), { reload: true }, { key: KEY_TURN_PICK(key), label: `正在切出「${LABEL_OF[key] ?? key}」第 ${index + 1} 帧…` });
            };
            ...
            })), h("div", { className: "SPR_axisRuler" }, h("span", null, "0 秒"), h("span", { className: "SPR_axisRulerMid" }, total > 0
                ? `共 ${total} 张候选帧 · 每张约 ${(duration / total).toFixed(2)} 秒 —— 拖动圆圈调整每个方向截在哪一帧`
                : "还没有候选帧：先点「生成转圈视频」，视频到手会自动抽帧"), h("span", null, `${duration.toFixed(2)} 秒`)));
        }
        ...
        function TurnImageStage({ project, api, start, setNotice, tasks, savePrompts }) {
            ...
            return h(React.Fragment, null, h("p", { className: "SPR_hint" }, "先生成一段「角色原地匀速转一整圈」的视频（绿幕、保持原图风格、除旋转外不做任何动作），再按时间截出八个方向的帧。八个方向出自同一段视频，一致性比逐方向生图好得多；下面时间轴上的八个圆圈就是各自的截帧位置，随便拖。"), h("textarea", {
                className: "SPR_area",
                value: promptDraft,
                "data-testid": "turn-prompt",
                onChange: (event) => setPromptDraft(event.target.value)
            }), h("div", { className: "SPR_toolbar" }, h(Btn, { disabled: !promptDirty, onClick: () => void savePrompts({ turn: promptDraft }) }, promptDirty ? "保存转圈提示词" : "转圈提示词已保存"), h(Btn, {
                onClick: () => void start(() => api.savePrompts({ projectId: project.id, resetTurnToDefault: true }), {
                    reload: true,
                    notice: "转圈提示词已重置为默认模板",
                    noticeKind: "ok"
                })
            }, "重置为默认")), h("div", { className: "SPR_toolbar" }, h(BusyBtn, {
            ...
            }, total > 0 ? `重新抽帧（当前 ${total} 张）` : "抽取候选帧"), h(NumField, {
                label: "候选帧数（8~64）",
                value: frameCount,
                min: TURN_FRAME_MIN,
                max: TURN_FRAME_MAX,
                ...
                onClick: () => void start(() => api.runTurnFrames({ projectId: project.id, count: frameCount }), { reload: true, notice: `已按 ${frameCount} 张重抽候选帧`, noticeKind: "ok" }, {
                    key: KEY_TURN_FRAMES,
                    label: `正在按 ${frameCount} 张重抽候选帧…`
                })
            }, "应用帧数"), h("span", { className: "SPR_refRow" }, project.settings?.turnFrameCount === frameCount ? `当前 ${project.settings?.turnFrameCount ?? "—"} 张` : `未应用（当前 ${project.settings?.turnFrameCount ?? "—"} 张）`)), h("div", { className: "SPR_toolbar" }, h(Btn, {
                disabled: total === 0 || cutting,
                onClick: () => void start(() => api.resetTurnPicks({ projectId: project.id }), { reload: true, notice: "八个圆圈已回到默认等分位置", noticeKind: "ok" })
            }, "重置八个圆圈"), h(Btn, {
                disabled: total === 0 || cutting,
            ...
            }, project.turn?.direction === "ccw" ? "转圈方向：逆时针（点一下换向）" : "转圈方向：顺时针（点一下换向）"), h(Btn, {
                on: allApproved,
            ...
            }, allApproved ? "取消全部通过" : "全部标记通过"), h(BusyBadge, { show: videoRunning || cutting, text: videoRunning ? videoLabel : cutLabel })), video.file !== undefined
                ...
                : h("div", { className: "SPR_thumbEmpty", style: { aspectRatio: "16/9" } }, videoRunning ? "正在生成转圈视频…" : "还没有转圈视频：点上面的「生成转圈视频」"), video.error !== undefined ? h("p", { className: "SPR_error" }, video.error) : null, h("div", { className: "SPR_refRow" }, `首帧用的是${video.firstFrame !== undefined && String(video.firstFrame).startsWith("images/") ? "已生成的正面绿幕图" : "上传的源图"}：${video.firstFrame ?? "—"}`), total > 0
                ? h(TurnAxis, {
                    project,
                    total,
                    duration,
                ...
                : h("div", { className: "SPR_thumbEmpty", style: { marginTop: 10 } }, cutting ? "正在抽取候选帧…" : stale ? "候选帧已过期（转圈视频换过了）：点「重新抽帧」" : "还没有候选帧"), frames.error !== undefined ? h("p", { className: "SPR_error" }, frames.error) : null, h("div", { className: "SPR_grid", style: { marginTop: 12 } }, DIRECTION_KEYS.map((key) => {
                ...
                return h("div", { key, className: "SPR_node", "data-busy": nodeBusy ? "true" : undefined }, h("div", { className: "SPR_nodeTop" }, h("span", { className: "SPR_nodeTitle" }, LABEL_OF[key] ?? key), nodeBusy ? h(Chip, { kind: "running", text: "切帧中" }) : h(StatusChip, { node })), node?.file !== undefined
                    ...
                    : h("div", { className: "SPR_thumbEmpty" }, nodeBusy ? "正在切出这一帧…" : "还没有这一方向的图"), node?.error !== undefined ? h("p", { className: "SPR_error" }, node.error) : null, h("span", { className: "SPR_refRow" }, total > 0
                    ? `第 ${index + 1}/${total} 帧 · ${((index * duration) / Math.max(1, total)).toFixed(2)} 秒`
                    : "等待候选帧"), h("div", { className: "SPR_btnRow" }, h(Btn, {
        ...
        function renderImageStage(ctx) {
            ...
            return h(React.Fragment, null, modeBar, h("div", { className: "SPR_toolbar" }, h(BusyBtn, {
                ...
                disabled: batchBusy,
                onClick: () => void start(() => api.setApproved({ projectId: project.id, stage: "images", approved: !allApproved }), { reload: true })
            }, allApproved ? "取消全部通过" : "全部标记通过"), h(BusyBtn, {
                busy: resetting,
                busyText: "正在重置…",
                disabled: batchBusy,
                onClick: () => {
                    if (typeof window !== "undefined" && !window.confirm("把八个方向的生图提示词和视频提示词都重置为当前默认模板？你手改过的内容会丢失。"))
                        return;
                    void start(() => api.savePrompts({ projectId: project.id, resetImagesToDefault: true, resetVideoToDefault: true }), { reload: true, notice: "提示词已重置为默认模板", noticeKind: "ok" }, { key: KEY_PROMPT_RESET, label: "正在重置提示词…" });
                }
            }, "重置提示词为默认"), h(BusyBadge, { show: batchBusy, text: batchLabel }), project.source === null ? h("span", { className: "SPR_refRow" }, "请先上传源图") : null), h("div", { className: "SPR_grid" }, DIRECTIONS.map((direction) => {
                ...
                return h("div", { key: direction.key, className: "SPR_node", "data-stale": node?.stale === true ? "true" : "false", "data-busy": nodeBusy ? "true" : undefined }, h("div", { className: "SPR_nodeTop" }, h("span", { className: "SPR_nodeTitle" }, direction.label), nodeBusy ? h(Chip, { kind: "running", text: "生成中" }) : h(StatusChip, { node })), node?.file !== undefined
                    ? h(MediaBox, { overlay: nodeBusy, text: overlayText }, h("img", {
                        className: "SPR_thumb",
                        src: assetUrl(project, node.file, node.updatedAt ?? project.updatedAt),
                        alt: direction.label
                    }))
                    : h("div", { className: "SPR_thumbEmpty" }, nodeBusy ? "正在生成…" : `参考：${direction.refs.join(" + ")}`), nodeBusy ? h(BusyBadge, { show: true, text: overlayText }) : null, node?.error !== undefined ? h("p", { className: "SPR_error" }, node.error) : null, node?.status === "ready" && node.elapsedMs !== undefined
                    ...
                    }, dirty ? "保存改动" : "已保存"), h(BusyBtn, {
                        busy: tasks.has(withPromptKey),
                        busyText: "正在提交…",
                        onClick: () => void start(() => api.runImage({ projectId: project.id, key: direction.key, prompt: draft }), { reload: true }, { key: withPromptKey, label: `正在用这段提示词生成「${direction.label}」…` })
                    }, "用这段提示词生成")))
                ...
                }, node?.file === undefined ? "生成" : "重新生成"), h(Btn, {
                    on: node?.approved === true,
        ...
        function renderVideoStage(ctx) {
            ...
            return h(React.Fragment, null, h("p", { className: "SPR_hint" }, "提示词要求「固定镜头、固定背景、原地走三步」。Hailuo 一段通常要 1~6 分钟，提交后可以离开这个页面。"), h("textarea", {
            ...
            }), h("div", { className: "SPR_toolbar" }, h(Btn, { disabled: !promptDirty, onClick: () => void savePrompts({ video: videoPromptDraft }) }, promptDirty ? "保存视频提示词" : "视频提示词已保存"), h(BusyBtn, {
                primary: true,
            ...
            }, `生成全部视频（${readyImages.length}/8 张绿幕图就绪）`), h(BusyBtn, {
                ...
                return h("div", { key: direction.key, className: "SPR_node", "data-busy": nodeBusy ? "true" : undefined }, h("div", { className: "SPR_nodeTop" }, h("span", { className: "SPR_nodeTitle" }, direction.label), nodeBusy ? h(Chip, { kind: "running", text: "生成中" }) : h(StatusChip, { node: video })), video?.file !== undefined
                    ...
                    : image?.file !== undefined
                        ...
                        : h("div", { className: "SPR_thumbEmpty" }, "还没有绿幕图"), nodeBusy || waiting ? h(BusyBadge, { show: true, text: overlayText }) : null, video?.remoteStatus !== undefined ? h("span", { className: "SPR_refRow" }, `远端状态：${video.remoteStatus}`) : null, video?.error !== undefined ? h("p", { className: "SPR_error" }, video.error) : null, h("div", { className: "SPR_btnRow" }, h(BusyBtn, {
        ...
        function renderFrameStage(ctx) {
            ...
            return h(React.Fragment, null, h("div", { className: "SPR_fields" }, h(NumField, {
                ...
                "预览带是**双击看大图**：格子尺寸只有 256px，看不清动作和抠像质量。"), h("div", { className: "SPR_toolbar" }, h(BusyBtn, {
                primary: true,
            ...
            }, allApproved ? "取消全部通过" : "全部标记通过"), h(BusyBadge, { show: extracting, text: tasks.label(KEY_FRAMES_ALL) ?? "正在抽帧…" })), h("div", { className: "SPR_grid" }, DIRECTIONS.map((direction) => {
                ...
                // 否则失败原因被遮罩挡住，看着像还在跑。
                const queued = (tasks.has(KEY_FRAMES_ALL) || jobCovers(hostExtract, direction.key)) && node?.status !== "error";
                ...
                return h("div", { key: direction.key, className: "SPR_node", "data-busy": nodeBusy ? "true" : undefined }, h("div", { className: "SPR_nodeTop" }, h("span", { className: "SPR_nodeTitle" }, direction.label), nodeBusy ? h(Chip, { kind: "running", text: "抽帧中" }) : h(StatusChip, { node })), node?.strip !== undefined
                    ...
                    : h("div", { className: "SPR_thumbEmpty" }, nodeBusy ? "正在抽帧…" : "尚未抽帧"), nodeBusy ? h(BusyBadge, { show: true, text: overlayText }) : null, node?.duration !== undefined
        ...
        function renderSheetStage(ctx) {
            ...
            return h(React.Fragment, null, h("p", { className: "SPR_hint" }, "抠像在这里会**再跑一次**：第 3 步那次抠的是每个方向的预览带（顺便把整图合出来），这里重抠是为了让下面这些参数改完立刻生效——两次读的是同一份帧缓存（raw.bin）、同一套参数，所以结果一致。" +
                "改完参数会自动重新抠像并合成整图，不需要重新抽帧。"), h("div", { className: "SPR_fields" }, h(NumField, {
                label: "整图单格宽（px）",
                value: draft.cellWidth ?? 256,
                min: 16,
                max: 2048,
            ...
            }, h("option", { value: "on" }, "开启（推荐：角色填满格子，八个方向缩放一致）"), h("option", { value: "off" }, "关闭（用整帧画面）")))), h("div", { className: "SPR_toolbar" }, h(BusyBtn, {
                ...
                }), { reload: true }, { key: KEY_SHEET_SAVE, label: "正在保存并重新合成…" })
            }, "保存并重新合成"), h(BusyBtn, {
            ...
            }, `合成整图（${framesReady}/8 组帧就绪）`), h(BusyBadge, { show: composing, text: composeLabel }), project.sheet?.file !== undefined
                ? h("a", {
                    className: "SPR_btn",
            ...
            }, project.sheet?.approved === true ? "整图已通过" : "整图通过")), project.sheet?.error !== undefined ? h("p", { className: "SPR_error" }, project.sheet.error) : null, h("div", { style: { display: "flex", gap: 18, alignItems: "flex-start", flexWrap: "wrap" } }, h("div", { className: "SPR_rowOrder" }, h("span", { className: "SPR_fieldLabel" }, "行序（第 1 行在最上方）"), rowOrder.map((key, index) => h("div", { key: `${key}-${index}`, className: "SPR_rowOrderItem" }, h("span", { className: "SPR_rowOrderIdx" }, String(index + 1)), h("span", { className: "SPR_rowOrderName" }, LABEL_OF[key] ?? key), h("button", { type: "button", className: "SPR_miniBtn", disabled: index === 0 || composing, onClick: () => moveRow(index, -1) }, "↑"), h("button", { type: "button", className: "SPR_miniBtn", disabled: index === rowOrder.length - 1 || composing, onClick: () => moveRow(index, 1) }, "↓")))), h("div", { style: { flex: 1, minWidth: 300 } }, project.sheet?.file !== undefined
                ? h(React.Fragment, null, h("p", { className: "SPR_hint" }, `输出 ${project.sheet.width}×${project.sheet.height} 像素 · 单格 ${draft.cellWidth}×${draft.cellHeight} · 每行 ${draft.frameCount ?? 8} 帧` +
                ...
                : h("div", { style: { position: "relative", minHeight: 160 } }, h("p", { className: "SPR_empty" }, composing ? "正在生成第一张整图…" : framesReady === 0 ? "请先完成第 3 步的抽帧" : "还没有合成整图"), h(LoadingOverlay, { show: composing, text: composeLabel })))));
        }
        ...
        function WalkPreview(props) {
            ...
            }, h("canvas", { ref: canvasRef, className: "SPR_canvas", width: STAGE_W, height: STAGE_H }), focused ? null : h("div", { className: "SPR_stageHint" }, "点击这里，然后用 WASD 或 ↑↓←→ 操控角色"), h(LoadingOverlay, {
                show: rebuilding || (ready && !sheetLoaded),
                text: rebuilding ? rebuildingText : "正在载入整图…"
            }), h("div", { className: "SPR_hud" }, h("span", { className: "SPR_hudDir" }, hud.compass), h("span", null, hud.moving ? "行走" : "站立"))), h("p", { className: "SPR_hint" }, "方向按屏幕方位映射：按 ↑ 向北走（背对镜头）、↓ 向南走（正对镜头）、← 向西、→ 向东；斜向同时按两个键。", h("br"), "「播放速度」只改步频快慢，不影响角色移动速度；「移动速度」只改走得多快，不影响动画帧率。切图用的是整图自己记录的行序，所以改完行序即使还没重新合成，预览也不会取错方向。"));
        }
        // ── 公共小工具 ──────────────────────────────────────────────────────
        ...
        function UploadBox({ label, accept, multiple, onFiles, busy }) {
            ...
            return h(React.Fragment, null, h("div", {
                ...
                ? h("span", { style: { display: "inline-flex", alignItems: "center", gap: 7, justifyContent: "center" } }, h("span", { className: "SPR_spinSm" }), "正在上传…")
                : label), h("div", { className: "SPR_toolbar" }, h(BusyBtn, { onClick: () => inputRef.current?.click(), busy: busy === true, busyText: "正在上传…" }, "选择文件"), h("input", {
                ref: inputRef,
                type: "file",
                accept,
        ...
        function ImageModule(props) {
            ...
            return h(React.Fragment, null, h("div", { className: "SPR_body" }, h(JobSidebar, {
                ...
                ? h("div", { className: "SPR_note", "data-kind": notice.kind }, notice.text, h("span", { style: { marginLeft: 10 } }, h("button", { type: "button", className: "SPR_miniBtn", onClick: () => setNotice(null) }, "关闭")))
                : null, h("div", { className: "SPR_toolbar" }, h(BusyBtn, { onClick: create, primary: true, busy: tasks.has(K_IMG_CREATE), busyText: "正在新建…" }, "新建任务"), job !== null ? h(Btn, { onClick: del, danger: true }, "删除任务") : null, h(BusyBadge, { show: tasks.active, text: tasks.label(K_IMG_JOB) ?? tasks.label(K_IMG_KEY) ?? tasks.label(K_IMG_UPLOAD) ?? tasks.firstLabel ?? "正在调用接口…" })), job === null
                ...
                : h(React.Fragment, null, h("div", { className: "SPR_card" }, h("div", { className: "SPR_cardHead" }, h("h3", null, "① 提示词")), h("p", { className: "SPR_hint" }, "统一附加提示词会接在主提示词后面，用来写跨批次的共同要求。"), h("textarea", {
                    ...
                    onClick: () => void run(() => api.runImageJob({ jobId: job.id }), `已开始生成 ${settingsDraft.count ?? 1} 张`, { key: K_IMG_JOB, label: `正在生成 ${settingsDraft.count ?? 1} 张图片…` })
                }, `生成 ${settingsDraft.count ?? 1} 张`), h("span", { className: "SPR_refRow" }, "按 Seedream 刊例约 0.2 元/张，实际以方舟账单为准"))), h("div", { className: "SPR_card" }, h("div", { className: "SPR_cardHead" }, h("h3", null, "② 参考图（可留空）")), h("p", { className: "SPR_hint" }, "最多 10 张。有参考图时走图生图；引用多张时可在提示词里写「图一」「图二」。"), h(UploadBox, { label: "把参考图拖到这里", accept: "image/*", multiple: true, busy: uploading || tasks.has(K_IMG_UPLOAD), onFiles: (files) => void upload(files, "ref") }), (job.refs ?? []).length === 0
                    ? null
                ...
                }, h("option", { value: "off" }, "不抠像"), h("option", { value: "on" }, "自动抠绿幕输出 PNG")), h(BusyBtn, {
                    busy: tasks.has(K_IMG_KEY),
                    busyText: "正在抠像…",
                    onClick: () => void run(() => api.keyImageJob({ jobId: job.id }), "已开始抠像", {
                        key: K_IMG_KEY,
                        label: "正在抠绿幕…"
                    })
                }, "按当前参数重新抠像")), h(KeyingFields, { draft: keyingDraft, onChange: (patch) => { const next = { ...keyingDraft, ...patch }; setKeyingDraft(next); void saveJob({ keying: patch }); } }), h(UploadBox, { label: "上传一张已有图片，直接抠成透明 PNG", accept: "image/*", multiple: false, busy: uploading || tasks.has(K_IMG_UPLOAD), onFiles: (files) => void upload(files, "item") }), tasks.has(K_IMG_UPLOAD) ? h(BusyBadge, { show: true, text: tasks.label(K_IMG_UPLOAD) }) : null), h("div", { className: "SPR_card" }, h("div", { className: "SPR_cardHead" }, h("h3", null, `④ 结果（${(job.items ?? []).length} 张）`)), h(ReviewModeBar, { api, module: "image", id: job.id, mode: job.reviewMode, onChanged: () => void load(job.id) }), (job.items ?? []).length === 0
                    ...
                    : h("div", { className: "SPR_grid" }, (job.items ?? []).map((item, index) => {
                        ...
                        return h("div", { key: index, className: "SPR_node", "data-busy": overlay ? "true" : undefined }, h("div", { className: "SPR_nodeTop" }, h("span", { className: "SPR_nodeTitle" }, `第 ${index + 1} 张`), overlay ? h(Chip, { kind: "running", text: "生成中" }) : h(StatusChip, { node: item }), item.source === "uploaded" ? h(Chip, { kind: "empty", text: "上传" }) : null), h(MediaBox, { overlay, text: overlayText }, item.keyedFile !== undefined
                            ...
                            : item.file !== undefined
                                ...
                                : h("div", { className: "SPR_thumbEmpty" }, overlay ? "生成中…" : "等待生成")), overlay ? h(BusyBadge, { show: true, text: overlayText }) : null, item.backgroundFraction !== undefined
                            ...
                            onToggle: () => void run(() => api.saveImageJob({ jobId: job.id, index, approved: item.approved !== true }), item.approved === true ? "已取消通过" : "已标记通过")
                        }), item.keyedFile !== undefined
                            ...
                            ? h(BusyBtn, {
                                ...
                                busyText: "正在提交…",
                                onClick: () => void run(() => api.runImageJob({ jobId: job.id, count: index + 1 }), "已重新生成", {
                                    key: itemKey,
                                    label: `正在重新生成第 ${index + 1} 张…`
                                })
                            }, "重新生成")
                            : null, h(Btn, { danger: true, disabled: overlay, onClick: () => void run(() => api.removeImageItem({ jobId: job.id, index }), "已删除") }, "删除")));
                    }))), h(JobLogPanel, { job })))));
        }
        // ── 模块③：序列帧生成 ───────────────────────────────────────────────
        ...
        function SequenceModule(props) {
            ...
            return h(React.Fragment, null, h("div", { className: "SPR_body" }, h(JobSidebar, {
                ...
                ? h("div", { className: "SPR_note", "data-kind": notice.kind }, notice.text, h("span", { style: { marginLeft: 10 } }, h("button", { type: "button", className: "SPR_miniBtn", onClick: () => setNotice(null) }, "关闭")))
                : null, h("div", { className: "SPR_toolbar" }, h(BusyBtn, { onClick: create, primary: true, busy: tasks.has(K_SEQ_CREATE), busyText: "正在新建…" }, "新建任务"), job !== null ? h(Btn, { onClick: del, danger: true }, "删除任务") : null, h(BusyBadge, { show: tasks.active, text: tasks.firstLabel ?? "正在调用接口…" })), job === null
                ...
                : h(React.Fragment, null, h("div", { className: "SPR_card" }, h("div", { className: "SPR_cardHead" }, h("h3", null, "① 视频输入")), h("p", { className: "SPR_hint" }, "平台规定两种模式互斥：首尾帧模式以一张图作为起始画面；多模态参考模式用参考图+参考视频来约束风格与动作。"), h("div", { className: "SPR_toolbar" }, h("select", {
                    ...
                    ? h(React.Fragment, null, h("span", { className: "SPR_fieldLabel" }, "首帧图（必填）"), h(RefRow, { job, frame: job.refs.firstFrame, kind: "firstFrame", api, reload: load, setNotice }), h("span", { className: "SPR_fieldLabel" }, "尾帧图（选填）"), h(RefRow, { job, frame: job.refs.lastFrame, kind: "lastFrame", api, reload: load, setNotice }), h(UploadBox, { label: "把首帧图拖到这里", accept: "image/*", busy: uploading || tasks.has(K_SEQ_UPLOAD), onFiles: (files) => void uploadRef(files, "firstFrame") }))
                    : h(React.Fragment, null, h("span", { className: "SPR_fieldLabel" }, `参考图（${(job.refs.referenceImages ?? []).length}/9）`), h("div", { className: "SPR_grid" }, (job.refs.referenceImages ?? []).map((ref) => h("div", { key: ref.file, className: "SPR_node" }, h("img", { className: "SPR_thumb", src: `${job.assetBase}${ref.file}?v=${job.updatedAt}`, alt: ref.name }), h("div", { className: "SPR_btnRow" }, h(Btn, { danger: true, onClick: () => void run(() => api.removeSequenceRef({ jobId: job.id, kind: "referenceImage", file: ref.file }), "已移除") }, "移除"))))), h(UploadBox, { label: "把参考图拖到这里", accept: "image/*", multiple: true, busy: uploading || tasks.has(K_SEQ_UPLOAD), onFiles: (files) => void uploadRef(files, "referenceImage") }), h("span", { className: "SPR_fieldLabel" }, `参考视频（${(job.refs.referenceVideos ?? []).length}/3，每段 2~15 秒，单文件 ≤ 40MB）`), h("div", { className: "SPR_toolbar" }, (job.refs.referenceVideos ?? []).map((ref) => h(Btn, { key: ref.file, danger: true, onClick: () => void run(() => api.removeSequenceRef({ jobId: job.id, kind: "referenceVideo", file: ref.file }), "已移除") }, `移除 ${ref.name}`))), h(UploadBox, { label: "把参考视频拖到这里", accept: "video/*", multiple: true, busy: uploading || tasks.has(K_SEQ_UPLOAD), onFiles: (files) => void uploadRef(files, "referenceVideo") }))), h("div", { className: "SPR_card" }, h("div", { className: "SPR_cardHead" }, h("h3", null, "② 提示词与参数")), h("textarea", {
                    className: "SPR_area",
                    value: promptDraft,
                    onChange: (event) => setPromptDraft(event.target.value),
                ...
                }, job.video?.status === "ready" ? "重新生成视频" : "生成视频"), h(BusyBtn, {
                    busy: tasks.has(K_SEQ_POLL),
                    busyText: "正在查询…",
                    onClick: () => void kickAndWatch(() => run(() => api.pollSequenceVideo({ jobId: job.id })), undefined, { key: K_SEQ_POLL, label: "正在查询远端进度…" })
                }, "立即刷新进度"), h(Btn, { danger: true, disabled: videoBusy, onClick: () => void run(() => api.clearSequenceVideo({ jobId: job.id }), "已清空视频与帧") }, "清空视频重来"), h("span", { className: "SPR_refRow" }, "H3 768P 按 0.5 元/秒刊例计费")), h("div", { className: "SPR_toolbar" }, h("span", { className: "SPR_refRow" }, `状态：${job.video?.status ?? "empty"} ${job.video?.remoteStatus ?? ""} ${job.video?.error ?? ""}`), h(BusyBadge, { show: videoBusy, text: videoText })), h("div", { style: { position: "relative", minHeight: job.video?.file === undefined ? 90 : undefined } }, job.video?.file !== undefined
                    ...
                    onClick: () => void kickAndWatch(() => api.runSequenceFrames({ jobId: job.id }), "已开始抽帧", {
                        key: K_SEQ_FRAMES,
                        label: "正在抽帧…"
                    })
                }, `按当前张数抽帧（${settingsDraft.frameCount ?? 8} 张）`), job.frames?.stale === true ? h(Chip, { kind: "stale", text: "参数已变，需重抽" }) : null, h(BusyBadge, { show: framesBusy, text: framesText }), h("span", { className: "SPR_refRow" }, job.frames?.duration !== undefined ? `视频时长 ${job.frames.duration.toFixed(2)} 秒 · ${job.frames.files.length} 帧` : ""))), h("div", { className: "SPR_card" }, h("div", { className: "SPR_cardHead" }, h("h3", null, "④ 绿幕抠像与合成")), h(KeyingFields, { draft: keyingDraft, onChange: (patch) => { const next = { ...keyingDraft, ...patch }; setKeyingDraft(next); void saveJob({ keying: patch }); } }), h("div", { className: "SPR_toolbar" }, h(BusyBtn, {
                    busy: tasks.has(K_SEQ_KEY),
                    busyText: "正在重新抠像…",
                    disabled: framesBusy,
                    onClick: () => void kickAndWatch(() => api.keySequenceFrames({ jobId: job.id }), "已开始重新抠像", {
                        key: K_SEQ_KEY,
                        label: "正在重新抠像…"
                    })
                }, "重新抠像"), h(BusyBtn, {
                    ...
                    busyText: "正在合成条图…",
                    disabled: framesBusy,
                    onClick: () => void kickAndWatch(() => api.composeSequence({ jobId: job.id }), "已合成", {
                        key: K_SEQ_COMPOSE,
                        label: "正在合成横向条图…"
                    })
                }, "合成横向条图"), job.sheet?.file !== undefined
                    ...
                    ? h("div", { className: "SPR_sheetWrap", style: { position: "relative" } }, h("img", { className: "SPR_sheet", src: `${job.assetBase}${job.sheet.file}?v=${job.sheet.updatedAt}`, alt: "序列帧条图", style: { visibility: composing ? "hidden" : undefined } }), h(LoadingOverlay, { show: composing, text: composeText }))
                    ...
                    })))), h("div", { className: "SPR_card" }, h("div", { className: "SPR_cardHead" }, h("h3", null, "⑤ 验收")), h("p", { className: "SPR_hint" }, "三步各自打「通过」。对话里的 agent 读写同一份标记：选「每一步人工审核」时，它会等你通过才继续。"), h(ReviewModeBar, { api, module: "sequence", id: job.id, mode: job.reviewMode, onChanged: () => void load(job.id) }), [
                    ...
                    onToggle: () => void run(() => api.saveSequenceJob({ jobId: job.id, step: entry.step, approved: entry.node?.approved !== true }), entry.node?.approved === true ? "已取消通过" : "已标记通过")
                })))), h(JobLogPanel, { job })))));
        }
        /** 单张参考图/首尾帧的展示行。 */
        ...
        function JobLogPanel({ job }) {
            ...
            return h("div", { className: "SPR_log", "data-testid": "rig-log" }, h("div", { className: "SPR_logHead" }, h("span", { className: "SPR_fieldLabel" }, `运行日志（${paired.length} 条${counts.error ? ` · ${counts.error} 错误` : counts.warn ? ` · ${counts.warn} 警告` : ""}）`), h("span", { className: "SPR_logFilters" }, h(Btn, { on: level === "all", "data-testid": "rig-log-all", onClick: () => setLevel("all") }, "全部"), h(Btn, { on: level === "warn", "data-testid": "rig-log-warn", onClick: () => setLevel("warn") }, `警告+（${(counts.warn ?? 0) + (counts.error ?? 0)}）`), h(Btn, { on: level === "error", "data-testid": "rig-log-error", onClick: () => setLevel("error") }, `错误（${counts.error ?? 0}）`), h("input", {
                className: "SPR_input",
                style: { width: 130 },
                placeholder: "搜关键字…",
                ...
                : h("ul", { className: "SPR_logList" }, entries.map((entry, index) => h("li", { key: `${entry.at}-${index}`, "data-level": entry.level }, h("span", { className: "SPR_logTime" }, new Date(entry.at).toLocaleTimeString("zh-CN", { hour12: false })), h("span", null, entry.message), entry.elapsed === undefined ? null : h("span", { className: "SPR_logElapsed" }, `（${formatElapsed(entry.elapsed)}）`)))));
        }
        ...
        function RigPartCard({ part, busy, onApprove, onHide, onRemove, onRetry, onSelect, onRename, selected }) {
            ...
            part.status === "ready" && (part.placed !== true || (typeof part.score === "number" && part.score < 0.5))
                ? h("div", { className: "SPR_rigCardMeta", style: { color: "var(--dsw-alias-state-warning-primary, #b45309)" } }, part.placed !== true ? "自动定位没找到，请手工拖到正确位置" : "相似度偏低，建议核对或拖一下")
                : null, part.error ? h("div", { className: "SPR_rigCardMeta", style: { color: "var(--dsw-alias-state-error-primary)" } }, part.error) : null, h("div", { className: "SPR_rigCardBtns" }, h(Btn, { onClick: onSelect, on: selected }, "选中"), h(Btn, { onClick: onApprove, on: part.approved === true }, part.approved ? "已通过" : "通过"), h(Btn, { onClick: onRetry, disabled: busy || part.status !== "ready" }, "重新定位"), h(Btn, { onClick: onRename }, "改名"), h(Btn, { onClick: onHide }, part.hidden ? "取消隐藏" : "隐藏"), h(Btn, { onClick: onRemove, danger: true }, "删除")));
        }
        // ── 手动装配编辑器 ──────────────────────────────────────────────────
        //
        ...
        function RigSemanticsPanel({ job, api, run, activeKey }) {
            ...
            const patch = (name, fields) => void run(() => api.setRigSemantics({ jobId: job.id, parts: [{ name, ...fields }], by: "human" }), `已更新「${name}」的语义`, activeKey);
            ...
            return h("div", { className: "SPR_rigStage", "data-testid": "rig-semantics", style: { marginTop: 12 } }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageHint", style: { flex: 1 } }, "语义（角色 / 父级 / 锚点）：决定骨骼挂在谁身上、骨骼从部件的哪一端伸到哪一端。改这里不会动②里已经摆好的位置，只会重算③骨骼。")), sem.errors.length > 0
                ? h("p", { className: "SPR_hint", style: { color: "var(--dsw-alias-state-error-primary)" } }, `语义有 ${sem.errors.length} 处错误：` + sem.errors.map((e) => `${e.name || "整体"} ${e.message}`).join("；"))
                : null, sem.warnings.length > 0
                ? h("p", { className: "SPR_hint", style: { color: "var(--dsw-alias-state-warning-primary, #b45309)" } }, `提示：` + sem.warnings.map((w) => `${w.name || "整体"} ${w.message}`).join("；"))
                : null, sem.errors.length === 0 && sem.warnings.length === 0
                ? h("p", { className: "SPR_hint" }, sem.ready ? "语义已就绪，无结构问题。" : "还没有可用部件。")
                : null, h("div", { style: { marginTop: 8, overflowX: "auto" } }, h("table", { className: "SPR_table", style: { width: "100%", fontSize: 12, borderCollapse: "collapse" } }, h("thead", null, h("tr", null, h("th", { style: { textAlign: "left" } }, "部件"), h("th", { style: { textAlign: "left" } }, "角色"), h("th", { style: { textAlign: "left" } }, "父级"), h("th", { style: { textAlign: "left" } }, "近端锚点"), h("th", { style: { textAlign: "left" } }, "远端锚点"), h("th", { style: { textAlign: "left" } }, "来源"))), h("tbody", null, parts.map((part) => h("tr", { key: part.name, "data-testid": `rig-sem-row-${part.name}` }, h("td", { style: { paddingRight: 8 } }, part.label ?? part.name), h("td", { style: { paddingRight: 8 } }, h("select", {
                className: "SPR_input",
            ...
            }, h("option", { value: "" }, "（挂 root）"), names.filter((name) => name !== part.name).map((name) => h("option", { key: name, value: name }, name)))), h("td", { style: { paddingRight: 8, whiteSpace: "nowrap" } }, anchorField(part, "proximal", 0), anchorField(part, "proximal", 1)), h("td", { style: { paddingRight: 8, whiteSpace: "nowrap" } }, anchorField(part, "distal", 0), anchorField(part, "distal", 1)), h("td", { style: { whiteSpace: "nowrap" } }, part.semanticsSource === "human" ? "人工" : part.semanticsSource === "ai" ? "AI" : "默认")))))));
        }
        /**
         * 骨骼编辑器（阶段③）——三通道模型的界面。
         *
        ...
        function RigBoneEditor({ job, api, run, busy, activeKey }) {
            ...
            if (bones.length === 0) {
                return h("p", { className: "SPR_hint", style: { marginTop: 10 } }, "还没有骨骼。先点上面的「生成骨骼与动画」——生成之后这里可以逐根微调。");
            }
            const manualCount = Object.keys(offsets).length;
            const patch = (name, fields) => void run(() => api.setRigBoneOffsets({ jobId: job.id, bones: [{ name, ...fields }], by: "human" }), `已调整「${name}」`, activeKey);
            return h("div", { "data-testid": "rig-bone-editor", style: { marginTop: 12 } }, h("h3", { style: { fontSize: 13, margin: "0 0 4px" } }, `骨骼手工偏移（${manualCount > 0 ? `已调 ${manualCount} 根` : "未调整"}）`), h("p", { className: "SPR_hint", style: { marginTop: 0 } }, "这里调的是「你相对绑定姿势改了多少」，不会被重新推骨骼覆盖。位移单位是参考图像素，旋转是度。全零即视为未调整。"), h("div", { style: { marginTop: 8, maxHeight: 320, overflowY: "auto" } }, h("table", { className: "SPR_table", style: { width: "100%", fontSize: 12, borderCollapse: "collapse" } }, h("thead", null, h("tr", null, h("th", { style: { textAlign: "left" } }, "骨骼"), h("th", { style: { textAlign: "left" } }, "父级"), h("th", { style: { textAlign: "left" } }, "绑定姿势"), h("th", { style: { textAlign: "left" } }, "手工位移 x"), h("th", { style: { textAlign: "left" } }, "手工位移 y"), h("th", { style: { textAlign: "left" } }, "手工旋转"), h("th", null, ""))), h("tbody", null, bones.filter((bone) => bone.name !== "root").map((bone) => {
                ...
                return h("tr", { key: bone.name, "data-testid": `rig-bone-row-${bone.name}` }, h("td", { style: { paddingRight: 8 } }, bone.name), h("td", { style: { paddingRight: 8, opacity: 0.7 } }, bone.parent ?? "root"), h("td", { style: { paddingRight: 8, opacity: 0.7, whiteSpace: "nowrap" } }, `${Math.round(bone.x)},${Math.round(bone.y)} · ${Math.round(bone.rotation)}°`), h("td", { style: { paddingRight: 8 } }, h(NumField, {
                    ...
                    : h("button", {
                        ...
                        onClick: () => void run(() => api.resetRigBoneOffsets({ jobId: job.id, names: [bone.name] }), `已清除「${bone.name}」的手工偏移`, activeKey)
                    }, "清除")));
            })))), manualCount > 0
                ? h("div", { className: "SPR_toolbar", style: { marginTop: 8 } }, h(BusyBtn, {
                    busy: busy,
                    busyText: "清除中…",
                    onClick: () => void run(() => api.resetRigBoneOffsets({ jobId: job.id }), "已清除全部手工骨骼偏移", activeKey)
        ...
        function RigAnimationPanel({ job, api, run, busy, activeKey }) {
            ...
            const patch = (id, fields) => void run(() => api.setRigAnimationSettings({ jobId: job.id, animations: [{ id, ...fields }], by: "human" }), `已调整「${id}」的动画参数`, activeKey);
            return h("div", { "data-testid": "rig-animation-panel", style: { marginTop: 12 } }, h("h3", { style: { fontSize: 13, margin: "0 0 4px" } }, `动画参数（${changed > 0 ? `已调 ${changed} 个` : "全部为预设默认值"}）`), h("p", { className: "SPR_hint", style: { marginTop: 0 } }, "幅度统一缩放旋转与位移（1 = 预设原样）；时长是一个循环的秒数。改完点上面的「生成骨骼与动画」重算。"), h("table", { className: "SPR_table", style: { width: "100%", fontSize: 12, borderCollapse: "collapse", marginTop: 8 } }, h("thead", null, h("tr", null, h("th", { style: { textAlign: "left" } }, "动作"), h("th", { style: { textAlign: "left" } }, "时长（秒）"), h("th", { style: { textAlign: "left" } }, "幅度"), h("th", { style: { textAlign: "left" } }, "状态"), h("th", null, ""))), h("tbody", null, presets.map((preset) => h("tr", { key: preset.id, "data-testid": `rig-anim-row-${preset.id}` }, h("td", { style: { paddingRight: 8 } }, `${preset.label}（${preset.id}）`), h("td", { style: { paddingRight: 8 } }, h(NumField, {
                label: "",
            ...
            })), h("td", { style: { paddingRight: 8, opacity: 0.8 } }, (preset.enabled ? "会生成" : "未勾选") +
                (preset.duration !== preset.defaultDuration ? ` · 预设 ${preset.defaultDuration}s` : "")), h("td", null, settings[preset.id] === undefined
                ? h("span", { style: { opacity: 0.4 } }, "—")
                : h("button", {
                    type: "button",
                    className: "SPR_miniBtn",
                    "data-testid": `rig-anim-reset-${preset.id}`,
                    onClick: () => void run(() => api.resetRigAnimationSettings({ jobId: job.id, ids: [preset.id] }), `已重置「${preset.id}」`, activeKey)
                }, "重置")))))), changed > 0
                ? h("div", { className: "SPR_toolbar", style: { marginTop: 8 } }, h(BusyBtn, {
                    busy: busy,
                    busyText: "重置中…",
                    onClick: () => void run(() => api.resetRigAnimationSettings({ jobId: job.id }), "已重置全部动画参数", activeKey)
                }, `重置全部（${changed} 个）`))
                : null);
        }
        ...
        function RigTimelineEditor({ job, api, run, busy, activeKey, onRebuild }) {
            ...
            React.useEffect(() => {
                ...
                const up = () => {
                    ...
                    setDraft((current) => {
                        if (current !== null) {
                            void run(() => api.saveRigAnimation({ jobId: job.id, id: current.id, animation: current }), `已移动「${drag.bone}/${TRACK_LABELS[drag.kind]}」的关键帧`, activeKey);
                        }
                        return current;
                    });
                };
            ...
            return h("div", { "data-testid": "rig-timeline", style: { marginTop: 12 } }, h("h3", { style: { fontSize: 13, margin: "0 0 4px" } }, "时间轴与关键帧"), h("p", { className: "SPR_hint", style: { marginTop: 0 } }, "选一台动画后可以加/删/拖关键帧、改数值与缓动。提交后这台动画就以这份数据为准（参数滑杆不再影响它），" +
                "「还原」可以退回预设。"), h("div", { className: "SPR_rigEditorRow" }, h("span", { className: "SPR_refRow" }, "动作"), presets.map((preset) => h(Btn, {
                key: preset.id,
                on: animId === preset.id,
                onClick: () => setAnimId(preset.id),
                "data-testid": `rig-tl-anim-${preset.id}`
            }, `${preset.label}${customIds.includes(preset.id) ? " ·已改" : ""}`))), draft === null
                ? h("p", { className: "SPR_hint" }, loading ? "读取中…" : "选择一台动画开始编辑。")
                : h("div", null, h("div", { className: "SPR_rigEditorRow" }, h("label", { className: "SPR_field" }, h("span", { className: "SPR_fieldLabel" }, "循环时长（秒）"), h("input", {
                    className: "SPR_input",
                    type: "number",
                    min: 0.1,
                    max: 10,
                ...
                }, draft.loop === true ? "循环" : "一次性"), h(Chip, { kind: customIds.includes(animId) ? "ready" : "idle", text: customIds.includes(animId) ? "已改为手工数据" : "预设（改动后转为数据）" }), customIds.includes(animId)
                    ? h(Btn, {
                ...
                // ── 选中帧的编辑 ────────────────────────────────────────
                selectedFrame === null
                    ? h("p", { className: "SPR_hint", style: { marginTop: 8 } }, "点一个关键帧来改它；在轨道空白处**双击**加一帧。首帧（0 秒）不能删——它是每条轨道的锚。")
                    : h("div", { className: "SPR_tlEdit", "data-testid": "rig-tl-edit" }, h("strong", null, `${selected.bone} · ${TRACK_LABELS[selected.kind]} · 第 ${selected.index + 1} 帧`), h(NumField, {
                        label: "时间(s)",
                    ...
                    })), h("div", { className: "SPR_rigEditorRow" }, h("span", { className: "SPR_refRow" }, "缓动"), EASING_PRESETS.map((preset) => h(Btn, {
                        key: preset.id,
                        on: easingNameOf(selectedFrame.frame.curve) === preset.id,
                        "data-testid": `rig-tl-ease-${preset.id}`,
                        onClick: () => setFrame({ curve: Array.isArray(preset.curve) ? [...preset.curve] : preset.curve }, `已把「${selected.bone}」的缓动改为 ${preset.label}`, autoRebuild && onRebuild !== undefined)
                    ...
                    }, "删除这一帧"), h("span", { className: "SPR_hint" }, selected.index === 0 ? "首帧是轨道锚点，不能删" : (selectedFrame.frames.length <= 2 ? "每条轨道至少留两帧" : "")))), 
                // ── 加轨道 ──────────────────────────────────────────────
                h("div", { className: "SPR_rigEditorRow", style: { marginTop: 8 } }, h("span", { className: "SPR_refRow" }, "加轨道"), h("select", {
                    className: "SPR_input",
                ...
                }, autoRebuild ? "改完自动重算预览：开" : "改完自动重算预览：关"), h(Btn, { onClick: () => void load(animId) }, "重新读取"), h("span", { className: "SPR_hint" }, "重算是本地计算，免费"))));
        }
        /**
         * 拆件质检面板（阶段①②之间）。
         *
        ...
        function RigQaPanel({ job, api, run, busy, activeKey }) {
            ...
            h("label", { className: "SPR_field" }, h("span", { className: "SPR_fieldLabel" }, "参考图里大约几个部位"), h("input", {
                className: "SPR_input",
                type: "number",
                min: 1,
                max: 200,
        ...
        function RigConstraintPanel({ job, api, run, busy, activeKey }) {
            ...
            return h("div", { "data-testid": "rig-constraints", style: { marginTop: 12 } }, h("h3", { style: { fontSize: 13, margin: "0 0 4px" } }, `IK 约束（${constraints.length > 0 ? `${constraints.length} 条` : "无"}）`), h("p", { className: "SPR_hint", style: { marginTop: 0 } }, "把一条骨骼链约束到一个**可拖的目标点**：在下面的预览里拖那个青色菱形，手就跟着走。" +
                "链长 1 = 末端骨 + 它的父级（两骨余弦定理）；权重小于 1 是软 IK。"), constraints.length > 0
                ? h("table", { className: "SPR_table", style: { width: "100%", fontSize: 12, borderCollapse: "collapse", marginTop: 8 } }, h("thead", null, h("tr", null, h("th", { style: { textAlign: "left" } }, "名称"), h("th", { style: { textAlign: "left" } }, "链末端骨骼"), h("th", { style: { textAlign: "left" } }, "目标点"), h("th", { style: { textAlign: "left" } }, "链长"), h("th", { style: { textAlign: "left" } }, "弯曲"), h("th", { style: { textAlign: "left" } }, "权重"), h("th", null, ""))), h("tbody", null, constraints.map((entry) => h("tr", { key: entry.name, "data-testid": `rig-ik-row-${entry.name}` }, h("td", null, entry.name), h("td", null, entry.bone), h("td", null, entry.target), h("td", null, h(NumField, {
                    label: "",
                    value: entry.chain,
                    min: 1,
                    max: 4,
                ...
                }, entry.bendPositive === true ? "正向" : "反向")), h("td", null, h(NumField, {
                    label: "",
                    value: entry.weight,
                    min: 0,
                    max: 1,
                ...
                : null, h("div", { className: "SPR_rigEditorRow", style: { marginTop: 8 } }, h("span", { className: "SPR_refRow" }, "加约束"), h("select", {
                className: "SPR_input",
                "data-testid": "rig-ik-bone",
                value: currentBone,
                onChange: (event) => setBone(event.target.value)
            }, boneChoices.map((name) => h("option", { key: name, value: name }, name))), h(NumField, { label: "链长", value: chain, min: 1, max: 4, step: 1, onChange: setChain }), h(Btn, {
                primary: true,
            ...
            }, "加到这条链上"), h("span", { className: "SPR_hint" }, "目标点会自动补一根骨骼，位置就在链末端")), constraints.length > 0
        ...
        function RigMeshPanel({ job, api, run, busy, activeKey }) {
            ...
            return h("div", { "data-testid": "rig-mesh", style: { marginTop: 12 } }, h("h3", { style: { fontSize: 13, margin: "0 0 4px" } }, `蒙皮网格（${targets.length > 0 ? `${targets.length} 个部件已细分` : "无"}）`), h("p", { className: "SPR_hint", style: { marginTop: 0 } }, "把部件切成网格之后就能对顶点做 FFD 变形——裙摆、披风、长发这类「上缘不动、下缘甩出去」" +
                "用刚体骨骼是动不出来的。网格是规则三角化，密度越高越软，预览也越吃性能。"), targets.length > 0
                ? h("table", { className: "SPR_table", style: { width: "100%", fontSize: 12, borderCollapse: "collapse", marginTop: 8 } }, h("thead", null, h("tr", null, h("th", { style: { textAlign: "left" } }, "部件"), h("th", { style: { textAlign: "left" } }, "列=行"), h("th", { style: { textAlign: "left" } }, "摆幅"), h("th", { style: { textAlign: "left" } }, "循环(s)"), h("th", { style: { textAlign: "left" } }, "固定端"), h("th", null, ""))), h("tbody", null, targets.map((name) => {
                    ...
                    return h("tr", { key: name, "data-testid": `rig-mesh-row-${name}` }, h("td", null, name), h("td", null, h(NumField, {
                        ...
                        }, h("option", { value: "top" }, "上缘"), h("option", { value: "bottom" }, "下缘"), h("option", { value: "none" }, "整体"))), h("td", null, h(Btn, {
                        danger: true,
                ...
                : null, h("div", { className: "SPR_rigEditorRow", style: { marginTop: 8 } }, h("span", { className: "SPR_refRow" }, "加网格"), h("select", {
                className: "SPR_input",
            ...
            }, "给裙摆加飘动"), h("span", { className: "SPR_hint" }, "网格与变形都是本地计算，免费")), targets.length > 0
                ? h("div", { className: "SPR_rigEditorRow" }, h(Btn, {
        ...
        function RigPathPanel({ job, api, run, busy, activeKey }) {
            ...
            return h("div", { "data-testid": "rig-paths", style: { marginTop: 12 } }, h("h3", { style: { fontSize: 13, margin: "0 0 4px" } }, `Path 约束（${entries.length > 0 ? `${entries.length} 条` : "无"}）`), h("p", { className: "SPR_hint", style: { marginTop: 0 } }, "把一串骨骼沿折线按弧长铺开——尾巴、辫子这类「长度远超单根骨」的部件用它。" +
                "间距 0 表示均匀铺满整条路径；大于 0 就是固定间距，骨骼只覆盖路径的一段。"), entries.length > 0
                ? h("table", { className: "SPR_table", style: { width: "100%", fontSize: 12, borderCollapse: "collapse", marginTop: 8 } }, h("thead", null, h("tr", null, h("th", { style: { textAlign: "left" } }, "名称"), h("th", { style: { textAlign: "left" } }, "骨链"), h("th", { style: { textAlign: "left" } }, "间距"), h("th", { style: { textAlign: "left" } }, "旋转混合"), h("th", null, ""))), h("tbody", null, entries.map(([name, entry]) => h("tr", { key: name, "data-testid": `rig-path-row-${name}` }, h("td", null, name), h("td", { style: { maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis" } }, entry.bones.join(" → ")), h("td", null, h(NumField, {
                ...
                : null, h("div", { className: "SPR_rigEditorRow", style: { marginTop: 8 } }, h("span", { className: "SPR_refRow" }, "加路径"), h("select", {
                className: "SPR_input",
                "data-testid": "rig-path-tip",
                value: currentTip,
                onChange: (event) => setTip(event.target.value)
            }, boneNames.map((name) => h("option", { key: name, value: name }, name))), h(NumField, { label: "链长", value: depth, min: 1, max: 6, step: 1, onChange: setDepth }), h(NumField, { label: "间距", value: spacing, min: 0, max: 400, step: 1, onChange: setSpacing }), h(Btn, {
                primary: true,
            ...
            }, "沿这条链加路径"), h("span", { className: "SPR_hint" }, "默认路径沿这些部件的近端锚点生成")), entries.length > 0
                ? h("div", { className: "SPR_rigEditorRow" }, h(Btn, {
        ...
        function RigTexturePanel({ job, api, run, busy, activeKey, selected, onSelect, keyBusy }) {
            ...
            const applyTint = () => void run(() => api.tintRigParts({ jobId: job.id, names: [current], tint: draft, by: "human", note: describeTint(draft) }), `已给「${current}」换色（新增一版）`, activeKey);
            return h("div", { className: "SPR_rigStage", "data-testid": "rig-texture-panel", style: { marginTop: 12 } }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, "贴图变体"), h("span", { className: "SPR_rigStageHint", style: { flex: 1 } }, "每一次换色 / 上传替换都新增一个版本，永不覆盖原图；「换回原版」就是切版本。")), h("div", { className: "SPR_rigEditorRow", style: { marginTop: 8 } }, h("span", { className: "SPR_refRow" }, "部件"), h("select", {
                className: "SPR_input",
                "data-testid": "rig-texture-part",
                value: current,
                onChange: (event) => pick(event.target.value)
            }, parts.map((entry) => h("option", { key: entry.name, value: entry.name }, entry.label ?? entry.name))), h("span", { className: "SPR_hint" }, `当前 v${activeVersion}${noteSuffix(versions.find((v) => v.v === activeVersion)?.note)}`)), 
            // 版本列表：点一下切过去，这就是「回退原版」。
            h("div", { className: "SPR_rigEditorRow", "data-testid": "rig-texture-versions" }, h("span", { className: "SPR_refRow" }, "版本"), versions.map((version) => h("span", { key: version.v, className: "SPR_refRow", style: { display: "inline-flex", alignItems: "center", gap: 4 } }, h("button", {
                type: "button",
                className: "SPR_miniBtn",
                ...
                onClick: () => void run(() => api.setRigTextureVersion({ jobId: job.id, name: current, version: version.v }), `已切到 v${version.v}`, activeKey)
            }, `v${version.v}${noteSuffix(version.note)}`), versions.length > 1 && version.v !== activeVersion
                ? h("button", {
                    type: "button",
                    className: "SPR_miniBtn",
                    title: "删除这一版",
                    onClick: () => void run(() => api.removeRigTextureVersion({ jobId: job.id, name: current, version: version.v }), `已删除 v${version.v}`, activeKey)
                }, "×")
                : null))), 
            // 即时预览 + 换色滑杆
            h("div", { className: "SPR_rigEditorRow", style: { alignItems: "flex-start", marginTop: 8 } }, part?.url !== null && part?.url !== undefined
                ...
                : null, h("div", { style: { display: "flex", flexWrap: "wrap", gap: 8, flex: 1 } }, h(RangeRow, { label: "色相", value: draft.hue, min: -180, max: 180, step: 5, onChange: (value) => setDraft({ ...draft, hue: value }) }), h(RangeRow, { label: "饱和度", value: draft.saturation, min: 0, max: 3, step: 0.1, onChange: (value) => setDraft({ ...draft, saturation: value }) }), h(RangeRow, { label: "明度", value: draft.lightness, min: -1, max: 1, step: 0.05, onChange: (value) => setDraft({ ...draft, lightness: value }) }), h(RangeRow, { label: "亮度", value: draft.brightness, min: -1, max: 1, step: 0.05, onChange: (value) => setDraft({ ...draft, brightness: value }) }), h(RangeRow, { label: "对比", value: draft.contrast, min: 0, max: 3, step: 0.1, onChange: (value) => setDraft({ ...draft, contrast: value }) }))), h("div", { className: "SPR_rigEditorRow" }, h(BusyBtn, {
                busy: busy,
                busyText: "换色中…",
                primary: true,
                disabled: isIdentity,
                ...
                onClick: applyTint
            }, "应用换色（新增一版）"), h(Btn, { onClick: () => setDraft(IDENTITY_TINT), disabled: isIdentity }, "重置滑杆"), h("label", { className: "SPR_refRow", style: { display: "inline-flex", gap: 6, alignItems: "center" } }, "上传替换", h("input", {
                type: "file",
                ...
                onChange: (event) => {
                    ...
                    reader.onload = () => {
                        ...
                        void run(() => api.uploadRigTexture({ jobId: job.id, name: current, data: base64, note: "手工上传" }), `已上传「${current}」的新贴图`, activeKey);
                    };
                    reader.readAsDataURL(file);
                }
            ...
            h("div", { className: "SPR_rigEditorRow", style: { marginTop: 6 } }, h("input", {
                ...
                onClick: () => void run(() => api.runRigRedraw({ jobId: job.id, name: current, prompt: redrawPrompt.trim() }), "已提交 AI 重绘（会花钱）", K_RIG_REDRAW)
            }, "AI 重绘这一块（会花钱）"), h("span", { className: "SPR_hint" }, "重绘结果是一版新的贴图：轮廓按原样裁回，不满意切回上一版即可")));
        }
        /** 把草稿说成人话，写进版本的 note（回退时能看懂哪一版是什么）。 */
        function describeTint(tint) {
            const bits = [];
            if (tint.hue)
                bits.push(`色相${tint.hue > 0 ? "+" : ""}${tint.hue}°`);
            if (tint.saturation !== 1)
                bits.push(`饱和×${tint.saturation}`);
            if (tint.lightness)
                bits.push(`明度${tint.lightness > 0 ? "+" : ""}${tint.lightness}`);
            if (tint.brightness)
                bits.push(`亮度${tint.brightness > 0 ? "+" : ""}${tint.brightness}`);
            if (tint.contrast !== 1)
                bits.push(`对比×${tint.contrast}`);
            return bits.join(" ") || "换色";
        }
        /**
         * 版本说明的后缀。
        ...
        function RigAssemblyEditor({ job, api, run, busy, activeKey }) {
            ...
            React.useEffect(() => {
                ...
                const onUp = (event) => {
                    ...
                    const point = toCanvas(event.clientX, event.clientY);
                    commit([{ name: drag.name, x: Math.round(point.x - width / 2), y: Math.round(point.y - height / 2), width, height, placed: true }], `已放置「${drag.name}」`);
            ...
            // ── 工具条 ────────────────────────────────────────────────
            h("div", { className: "SPR_asmBar" }, h("span", { className: "SPR_asmGroup" }, h("span", { className: "SPR_fieldLabel" }, "缩放"), h(Btn, { onClick: () => zoomCenter(1 / RIG_ZOOM_STEP), title: "缩小" }, "−"), h(Btn, { onClick: fitView, "data-testid": "rig-asm-fit", title: "适应窗口（Ctrl/Cmd+0）" }, `${Math.round(zoom * 100)}%`), h(Btn, { onClick: () => zoomCenter(RIG_ZOOM_STEP), title: "放大" }, "+")), h("span", { className: "SPR_asmGroup" }, h(Btn, { on: showReference, onClick: () => setShowReference((v) => !v), title: "把参考图叠在下面，方便对位" }, "参考图底图"), h(Btn, { on: showBoxes, onClick: () => setShowBoxes((v) => !v) }, "显示边框"), h(Btn, { on: snapEnabled, onClick: () => setSnapEnabled((v) => !v), title: "拖动时吸附到其它部件的边与中线" }, "吸附"), h(Btn, { on: lockRatio, onClick: () => setLockRatio((v) => !v), title: "拖角手柄时保持宽高比" }, "锁等比")), h("span", { className: "SPR_asmGroup" }, h(Btn, { onClick: undo, disabled: history.length === 0 }, `撤销${history.length > 0 ? `(${history.length})` : ""}`), h(Btn, { onClick: redo, disabled: future.length === 0 }, "重做")), h("span", { className: "SPR_spacer" }), h("span", { className: "SPR_asmHint" }, `已放置 ${placedNames.length}/${(job.parts ?? []).filter((part) => part.status === "ready").length}　·　拖部件移动、拖角缩放、方向键微调 1px（Shift 10px）、Delete 收回、滚轮缩放、空格拖动平移`)), h("div", { className: "SPR_asmBody" }, 
            // ── 画布 ──────────────────────────────────────────────
            h("div", {
                ...
                : null)), 
            // ── 右侧：部件栏 + 参数 ─────────────────────────────────
            h("div", { className: "SPR_asmSide" }, h("div", { className: "SPR_sideTitle" }, `未放置（${unplaced.length + notYetPlaced.length}）· 拖进画布`), h("div", { className: "SPR_asmPalette" }, [...unplaced, ...notYetPlaced].map((part) => h("div", {
                key: `palette:${part.name}`,
                className: "SPR_asmChip",
                onMouseDown: (event) => startPaletteDrag(part, event)
            }, part.url !== null && part.url !== undefined ? h("img", { src: part.url, alt: part.name, draggable: false }) : null, h("span", null, part.label ?? part.name))), unplaced.length + notYetPlaced.length === 0 ? h("p", { className: "SPR_hint" }, "全部部件都已放置。") : null), selectedItem !== undefined && selected !== null
                ? h("div", { className: "SPR_asmPanel" }, h("div", { className: "SPR_sideTitle" }, `选中：${selectedPart?.label ?? selected}`), h("div", { className: "SPR_asmFields" }, h(NumField, { label: "x", value: selectedItem.x, onChange: (value) => patchSelected({ x: value }, "已修改 x") }), h(NumField, { label: "y", value: selectedItem.y, onChange: (value) => patchSelected({ y: value }, "已修改 y") }), h(NumField, { label: "宽", value: selectedItem.width, min: 1, onChange: (value) => patchSelected(lockRatio ? { width: value, height: Math.round((value * selectedItem.height) / Math.max(1, selectedItem.width)) } : { width: value }, "已修改宽度") }), h(NumField, { label: "高", value: selectedItem.height, min: 1, onChange: (value) => patchSelected(lockRatio ? { height: value, width: Math.round((value * selectedItem.width) / Math.max(1, selectedItem.height)) } : { height: value }, "已修改高度") }), h(NumField, { label: "旋转", value: selectedItem.rotation, step: 5, onChange: (value) => patchSelected({ rotation: value }, "已旋转") }), h(NumField, { label: "层级", value: selectedItem.z, onChange: (value) => patchSelected({ z: value }, "已修改层级") })), h("div", { className: "SPR_asmRow" }, h(Btn, { onClick: () => changeZ(selected, "front") }, "置顶"), h(Btn, { onClick: () => changeZ(selected, "up") }, "上移"), h(Btn, { onClick: () => changeZ(selected, "down") }, "下移"), h(Btn, { onClick: () => changeZ(selected, "back") }, "置底")), h("div", { className: "SPR_asmRow" }, h(Btn, { onClick: () => patchSelected({ width: selectedItem.height, height: selectedItem.width }, "已交换宽高") }, "宽高互换"), h(BusyBtn, {
                    busy: busy,
                    busyText: "重定位中…",
                    onClick: () => void run(() => api.runRigLayout({ jobId: job.id, names: [selected] }), `已重新自动定位「${selected}」`, activeKey)
                }, "这块重新自动定位"), h(Btn, { onClick: () => commit([{ name: selected, placed: false }], `已收回「${selected}」`), danger: true }, "收回部件")), selectedItem.matched === false
                    ? h("p", { className: "SPR_hint" }, "这块目前是「未放置」状态：拖到画布上或点上面的数值确认即可放回。")
                    : null)
                : h("div", { className: "SPR_asmPanel" }, h("p", { className: "SPR_hint" }, "在画布上点一个部件，这里会出现它的精确参数与图层操作。")), h("div", { className: "SPR_asmPanel" }, h("div", { className: "SPR_sideTitle" }, "图层（从下到上）"), h("ol", { className: "SPR_asmLayers" }, placedNames.slice().reverse().map((name) => h("li", {
                key: `layer-row:${name}`,
        ...
        function RigExperimentalDialog({ onClose }) {
            ...
            return h("div", {
                ...
                onClick: close
            }, h("div", { className: "SPR_gate", onClick: (event) => event.stopPropagation() }, h("div", { className: "SPR_gateHead" }, h("span", { className: "SPR_expTag" }, "实验性"), h("span", { className: "SPR_gateTitle" }, "骨骼动画生成仍在开发中")), RIG_EXPERIMENTAL_POINTS.map((text, index) => h("p", { key: index, className: "SPR_gateBody" }, text)), h("a", { className: "SPR_link", href: RIG_EXPERIMENTAL_REPO, target: "_blank", rel: "noreferrer" }, RIG_EXPERIMENTAL_REPO), h("div", { className: "SPR_gateFoot" }, h("label", { className: "SPR_gateMute" }, h("input", { type: "checkbox", checked: mute, onChange: (event) => setMute(event.target.checked) }), "本次会话不再提示"), h("a", { className: "SPR_btn", href: RIG_EXPERIMENTAL_REPO, target: "_blank", rel: "noreferrer" }, "去 GitHub 仓库"), h(Btn, { onClick: close, primary: true, "data-testid": "rig-experimental-ok" }, "我知道了"))));
        }
        /** 模块内常驻的实验性提示条：弹窗关掉之后，标记不能跟着消失。 */
        function RigExperimentalBar() {
            return h("div", { className: "SPR_expBar" }, h("span", { className: "SPR_expTag" }, "实验性"), h("span", { className: "SPR_expBarText" }, "该模块功能尚不完善，仍在开发中；若你需要它，欢迎到"), h("a", { className: "SPR_link", href: RIG_EXPERIMENTAL_REPO, target: "_blank", rel: "noreferrer" }, "GitHub 仓库"), h("span", { className: "SPR_expBarText" }, "一起开发。"));
        }
        /**
         * 模块④的主体界面。
         *
        ...
        function RigModule(props) {
            ...
            const createJob = () => run(async () => {
                const created = await api.createRigJob({ name: "新骨骼动画任务" });
                await refresh();
                setJobId(created.jobId);
                setStage("parts");
                return created;
            }, "已新建骨骼动画任务", K_RIG_CREATE);
            const deleteJob = async () => {
                if (job === null)
                    return;
                if (typeof window !== "undefined" && !window.confirm(`删除任务「${job.name}」？产物文件会一并删除。`))
                    return;
                await run(async () => {
            ...
            const saveSettings = () => run(() => api.saveRigJob({ jobId: job.id, sheetPrompt: promptDraft, suffix: suffixDraft, settings: settingsDraft }), "已保存提示词与参数");
            ...
            const headerButtons = h(React.Fragment, null, h(Btn, { onClick: () => void refresh(), disabled: loading || keyBusy(K_RIG_JOB) }, "刷新列表"), job !== null ? h(Btn, { onClick: () => void renameJob() }, "重命名") : null, job !== null ? h(Btn, { onClick: () => void deleteJob(), danger: true }, "删除任务") : null, h(BusyBtn, { onClick: () => void createJob(), busy: keyBusy(K_RIG_CREATE), busyText: "创建中…", primary: true }, "新建任务"));
            return h("div", { className: "SPR_rigPanel" }, 
            // 弹窗是 position:fixed，放在这个滚动容器里也照样盖住整个工作台，
            // 不必为此把 RigModule 再包一层 Fragment。
            gateOpen ? h(RigExperimentalDialog, { onClose: closeGate }) : null, h(RigExperimentalBar, null), h("div", { className: "SPR_toolbar" }, h("span", { className: "SPR_refRow" }, "生图模型"), h("span", { className: "SPR_badge" }, globalConfig?.arkModel ?? job?.settings?.model ?? "未配置"), h("span", { className: "SPR_refRow" }, "（模型在「设置 → 游戏素材大师」里改）"), h("span", { className: "SPR_spacer" }), headerButtons), notice !== null
                ? h("div", { className: "SPR_notice", "data-kind": notice.kind === "error" ? "error" : "info" }, notice.text, h(Btn, { onClick: () => setNotice(null) }, "关闭"))
                ...
                }, h("span", { className: "SPR_moduleTitle" }, `${done ? "✓ " : ""}${entry.title}`), h("span", { className: "SPR_moduleHint" }, info.status === "running" ? "进行中…" : entry.hint));
            })), jobs.length === 0
                ? h("p", { className: "SPR_hint" }, loading ? "正在读取任务…" : "还没有骨骼动画任务，点右上角「新建任务」开始。")
                : h(React.Fragment, null, h("div", { className: "SPR_toolbar", style: { flexWrap: "wrap" } }, h("span", { className: "SPR_refRow" }, "任务"), h("select", { className: "SPR_input", style: { width: 260 }, value: jobId ?? "", onChange: (event) => setJobId(event.target.value) }, jobs.map((entry) => h("option", { key: entry.id, value: entry.id }, `${entry.name}（部件 ${entry.partCount}）`))), job !== null ? h("span", { className: "SPR_badge" }, job.id) : null, stageBusy ? h(BusyBadge, { show: true, text: "本阶段进行中…" }) : null), job === null
                    ? null
                    ...
                    // ── 第 0 步：角色参考图 ────────────────────────────────
                    h("div", { className: "SPR_rigStage" }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, "角色参考图"), h("span", { className: "SPR_rigStageHint" }, "拆件、装配、骨骼都以这张整图为基准；建议用能看清全身、背景干净的角色立绘。"), job.sourceUrl !== null && job.sourceUrl !== undefined ? h(Chip, { kind: "ready", text: `${job.canvas?.width ?? "?"}×${job.canvas?.height ?? "?"}` }) : h(Chip, { kind: "empty", text: "未上传" })), h("div", { className: "SPR_toolbar", style: { marginTop: 10 } }, h(UploadBox, { label: "拖入角色整图（PNG / JPG）", accept: "image/*", onFiles: (files) => void uploadSource(files), busy: uploading }), h(UploadBox, { label: "拖入部件 PNG（可多选，文件名即部件名）", accept: "image/*", multiple: true, onFiles: (files) => void uploadParts(files), busy: uploading })), job.sourceUrl !== null && job.sourceUrl !== undefined
                        ? h("img", { className: "SPR_rigCardImg", style: { height: 200, marginTop: 10 }, src: job.sourceUrl, alt: "reference" })
                        : null), 
                    // ── ① 拆件 ────────────────────────────────────────────
                    stage === "parts"
                        ? h("div", { className: "SPR_rigStage" }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, "① 拆件"), h("span", { className: "SPR_rigStageHint" }, "让生图模型把角色拆成摊平的部件图，再自动分割成逐件透明 PNG。**这一步花钱**，只跑一次；参数改了可以「重新分割」，不额外计费。"), h(StatusChip, { node: rigStageOf(job, "parts") })), h("div", { className: "SPR_rigEditorRow" }, h(BusyBtn, {
                        ...
                        }, job.sheet?.status === "ready" ? "重新生成拆件图（会花钱）" : "生成拆件图（会花钱）"), h(BusyBtn, {
                        ...
                        }, "用现有拆件图重新分割"), h(NumField, { label: "网格列", value: settingsDraft.gridColumns ?? 4, min: 1, max: 8, onChange: (value) => setSettingsDraft({ ...settingsDraft, gridColumns: value }) }), h(NumField, { label: "网格行", value: settingsDraft.gridRows ?? 4, min: 1, max: 8, onChange: (value) => setSettingsDraft({ ...settingsDraft, gridRows: value }) }), h(NumField, { label: "底色容差", value: settingsDraft.backgroundTolerance ?? 30, min: 1, max: 200, onChange: (value) => setSettingsDraft({ ...settingsDraft, backgroundTolerance: value }) }), h(NumField, { label: "边缘羽化", value: settingsDraft.feather ?? 26, min: 1, max: 200, onChange: (value) => setSettingsDraft({ ...settingsDraft, feather: value }) }), h(NumField, { label: "最小面积", value: settingsDraft.minArea ?? 0, min: 0, max: 100000, onChange: (value) => setSettingsDraft({ ...settingsDraft, minArea: value }) }), 
                        // 「边缘羽化」以前是「离底色多远才不透明」的斜率，浅色填充会
                        // 因此整片变半透明；现在它只作用于轮廓最外圈，内部一律不透明。
                        h("p", { className: "SPR_hint", style: { marginTop: 4, marginBottom: 0 } }, "底色容差：与底色多接近算背景。边缘羽化：只决定轮廓最外圈那几个像素的软过渡，部件内部不会变半透明。"), h(Btn, { onClick: () => void saveSettings() }, "保存参数与提示词")), h("textarea", {
                            className: "SPR_input",
                            ...
                            ? h("div", { className: "SPR_rigCanvasWrap" }, h("img", { src: job.sheet.url, alt: "sheet" }), h(LoadingOverlay, { show: keyBusy(K_RIG_SHEET) || keyBusy(K_RIG_SEGMENT), text: keyBusy(K_RIG_SHEET) ? "生图模型正在拆件…" : "正在分割部件…" }))
                            : h("p", { className: "SPR_hint", style: { marginTop: 10 } }, job.sourceUrl === null || job.sourceUrl === undefined ? "先上传角色参考图。" : "还没有拆件图：点上面的「生成拆件图」，或者用上面的上传框直接给现成部件 PNG。"), (job.parts ?? []).length > 0
                            ? h("div", null, h("h3", { style: { fontSize: 13, margin: "14px 0 0" } }, `部件（${job.parts.length}）——逐件验收，摆错的可单独重跑`), h("div", { className: "SPR_rigGrid" }, job.parts.map((part) => h(RigPartCard, {
                                key: part.name,
                                part,
                                ...
                                onRename: () => {
                                    const next = typeof window === "undefined" ? null : window.prompt(`把「${part.name}」改成什么名字？\n（名字决定骨骼层级，标准名如 head / torso / hip / left-upper-arm / left-lower-leg / right-foot，见拆件提示词里的网格表）`, part.name);
                                    if (next === null || next.trim() === "" || next === part.name)
                                        return;
                                    void run(() => api.renameRigPart({ jobId: job.id, from: part.name, to: next.trim() }), `已改名为「${next.trim()}」`);
                                },
                                onRetry: () => void run(() => api.runRigLayout({ jobId: job.id, names: [part.name] }), `已重新定位「${part.name}」`, PART_K(part.name, "relayout"))
                            }))))
                        ...
                        }, rigStageOf(job, "parts").approved === true ? "第①步：已通过" : "第①步：通过"), h(Btn, {
                            onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: false, stage: "parts" }))
                        }, "取消通过"), h(Btn, { onClick: () => setStage("layout") }, "下一步：装配定位 →")))
                        : null, 
                    // ── ② 装配定位 ────────────────────────────────────────
                    stage === "layout"
                        ? h("div", { className: "SPR_rigStage" }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, "② 装配定位"), h("span", { className: "SPR_rigStageHint" }, "多尺度模板匹配把每个部件摆回参考姿态。**本地计算，免费**：失败或摆错只重跑那几个部件，不用整批重来。"), h(StatusChip, { node: rigStageOf(job, "layout") })), h("div", { className: "SPR_rigEditorRow" }, h(BusyBtn, {
                        ...
                        }, `只重试未命中的 ${(job.review?.unmatched ?? []).length} 个`), h(Btn, { onClick: () => setShowReference((current) => !current), on: showReference }, showReference ? "只显示合成图" : "并排显示参考图")), h(RigAssemblyEditor, { job, api, run, busy, activeKey: K_RIG_LAYOUT }), (job.review?.unmatched ?? []).length > 0
                            ...
                            : null, h("div", { className: "SPR_toolbar", style: { marginTop: 12 } }, h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: true, stage: "layout" }), "第②步已通过"), on: rigStageOf(job, "layout").approved === true, disabled: job.layout?.status !== "ready" }, rigStageOf(job, "layout").approved === true ? "第②步：已通过" : "第②步：通过"), h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: false, stage: "layout" })) }, "取消通过"), h(Btn, { onClick: () => setStage("rig") }, "下一步：骨骼与动画 →")))
                        : null, 
                    // ── ③ 骨骼与动画 ──────────────────────────────────────
                    stage === "rig"
                        ? h("div", { className: "SPR_rigStage" }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, "③ 骨骼与动画"), h("span", { className: "SPR_rigStageHint" }, "按部件语义自动推骨骼层级，并生成六个动画预设。**本地计算，免费**。下面直接播放验收。"), h(StatusChip, { node: rigStageOf(job, "rig") })), h("div", { className: "SPR_rigEditorRow" }, h(BusyBtn, {
                        ...
                        }, "生成骨骼与动画"), h("span", { className: "SPR_refRow" }, "动画预设"), RIG_ANIMATIONS.map((name) => h("label", { key: name, className: "SPR_refRow", style: { display: "inline-flex", gap: 4, alignItems: "center" } }, h("input", {
                            type: "checkbox",
                        ...
                        }), name)), h(Btn, { onClick: () => void saveSettings() }, "保存动画选择"), job.rig?.skeleton !== null && job.rig?.skeleton !== undefined ? h("a", { className: "SPR_link", href: job.rig.skeleton, download: "skeleton.json" }, "下载 skeleton.json") : null, job.rig?.preview !== null && job.rig?.preview !== undefined ? h("a", { className: "SPR_link", href: job.rig.preview, target: "_blank", rel: "noreferrer" }, "新窗口打开预览") : null), (job.rig?.warnings ?? []).length > 0
                            ...
                            ? h(React.Fragment, null, h("div", { className: "SPR_rigEditorRow" }, h("span", { className: "SPR_refRow" }, "播放动画"), (job.rig.animations ?? []).map((name) => h(Btn, { key: name, on: anim === name, onClick: () => setAnim(name) }, name)), h("span", { className: "SPR_hint" }, "预览里也能拖时间轴、开骨骼网格")), h("iframe", {
                            ...
                            : h("p", { className: "SPR_hint", style: { marginTop: 10 } }, job.layout?.status === "ready" ? "点「生成骨骼与动画」得到 skeleton.json 与可播放预览。" : "先完成第②步装配定位。"), h(RigAnimationPanel, { job, api, run, busy, activeKey: K_RIG_BONES }), h(RigTimelineEditor, { job, api, run, busy, activeKey: K_RIG_BONES, onRebuild: true }), h(RigConstraintPanel, { job, api, run, busy, activeKey: K_RIG_BONES }), h(RigMeshPanel, { job, api, run, busy, activeKey: K_RIG_BONES }), h(RigPathPanel, { job, api, run, busy, activeKey: K_RIG_BONES }), h(RigBoneEditor, { job, api, run, busy, activeKey: K_RIG_BONES }), 
                        ...
                        job.rig?.dragonBones?.skeleton !== null && job.rig?.dragonBones?.skeleton !== undefined
                            ? h("div", { className: "SPR_rigEditorRow", "data-testid": "rig-bones-db" }, h("span", { className: "SPR_refRow" }, "DragonBones 骨架"), h("a", { className: "SPR_link", href: job.rig.dragonBones.skeleton, target: "_blank", rel: "noreferrer" }, "skeleton_ske.json"), h("span", { className: "SPR_hint" }, "同一份骨架的 DragonBones 5.5 写法（Cocos / Egret / Laya 可直接加载）"))
                            : null, h("div", { className: "SPR_toolbar", style: { marginTop: 12 } }, h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: true, stage: "rig" }), "第③步已通过"), on: rigStageOf(job, "rig").approved === true, disabled: job.rig?.status !== "ready" }, rigStageOf(job, "rig").approved === true ? "第③步：已通过" : "第③步：通过"), h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: false, stage: "rig" })) }, "取消通过"), h(Btn, { onClick: () => setStage("atlas") }, "下一步：图集 →")))
                        : null, 
                    // ── ④ 图集 ────────────────────────────────────────────
                    stage === "atlas"
                        ? h("div", { className: "SPR_rigStage" }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, "④ 图集"), h("span", { className: "SPR_rigStageHint" }, "把部件按装配后的尺寸打包成 Spine 纹理图集。区域尺寸与 skeleton.json 里挂点的 width/height 一致，导入引擎不会错位。"), h(StatusChip, { node: rigStageOf(job, "atlas") })), h("div", { className: "SPR_rigEditorRow" }, h(BusyBtn, {
                        ...
                        }, "打包纹理图集"), job.atlas?.text !== null && job.atlas?.text !== undefined ? h("a", { className: "SPR_link", href: job.atlas.text, target: "_blank", rel: "noreferrer" }, "查看 skeleton.atlas") : null, job.atlas?.image !== null && job.atlas?.image !== undefined ? h("a", { className: "SPR_link", href: job.atlas.image, download: "skeleton.png" }, "下载 skeleton.png") : null, job.atlas?.width !== undefined ? h(Chip, { kind: "ready", text: `${job.atlas.width}×${job.atlas.height} · ${job.atlas.regions} 区域${(job.atlas.pages ?? []).length > 1 ? ` · ${job.atlas.pages.length} 页` : ""}` }) : null), 
                        ...
                        (job.atlas?.dragonBones ?? []).length > 0
                            ? h("div", { className: "SPR_rigEditorRow", "data-testid": "rig-atlas-db" }, h("span", { className: "SPR_refRow" }, "DragonBones 贴图"), job.atlas.dragonBones.map((entry) => h("a", { key: entry.file, className: "SPR_link", href: entry.url, target: "_blank", rel: "noreferrer" }, entry.file.split("/").pop())))
                            : null, (job.atlas?.warnings ?? []).length > 0
                            ? h("p", { className: "SPR_hint", style: { color: "var(--dsw-alias-state-warning-primary, #b45309)" } }, `图集提示：${job.atlas.warnings.join("；")}`)
                        ...
                        (job.atlas?.pages ?? []).length > 1
                            ? h("div", { className: "SPR_rigEditorRow" }, h("span", { className: "SPR_refRow" }, "分页"), job.atlas.pages.map((page) => h("a", { key: page.file, className: "SPR_link", href: `/${page.file}`, target: "_blank", rel: "noreferrer" }, `${page.file.split("/").pop()}（${page.width}×${page.height} · ${page.regions} 区域）`)))
                            : null, h("div", { className: "SPR_toolbar", style: { marginTop: 12 } }, h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: true, stage: "atlas" }), "第④步已通过"), on: rigStageOf(job, "atlas").approved === true, disabled: job.atlas?.status !== "ready" }, rigStageOf(job, "atlas").approved === true ? "第④步：已通过" : "第④步：通过"), h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: false, stage: "atlas" })) }, "取消通过")), h("p", { className: "SPR_hint", style: { marginTop: 10 } }, "导入 Spine：把 skeleton.json、skeleton.atlas、skeleton.png 三个文件放在同一目录，打开 Spine 时选 skeleton.json 即可。"), h("p", { className: "SPR_hint", style: { marginTop: 4 } }, "导入 DragonBones：把 export/dragonbones/skeleton_ske.json 与 atlas/skeleton_tex.json、atlas/skeleton.png 放同一目录后加载 .json 数据与纹理。"))
        ...
        function ConfigSection(props) {
             ...
             * 需要时在这里单独指定。
             */
            h("label", { className: "SPR_field" }, h("span", { className: "SPR_fieldLabel" }, "部件重绘模型"), h("select", {
                className: "SPR_input",
                "data-testid": "cfg-redraw-model",
                value: config.arkRedrawModel ?? "",
                onChange: (event) => void patch({ arkRedrawModel: event.target.value })
            }, h("option", { value: "" }, "跟生图模型相同"), (config.arkModels ?? []).map((model) => h("option", { key: model.id, value: model.id }, `${model.label}（${model.id}）`)))), h("label", { className: "SPR_field" }, h("span", { className: "SPR_fieldLabel" }, "输出尺寸"), h("input", {
                className: "SPR_input",
            ...
            }, "测试连接（会真实生成 1 张 1K 小图，产生少量费用）"))), h("div", { className: "SPR_settingsGroup" }, h("h3", null, "MiniMax（图生视频）"), h("p", null, "视频阶段使用图生视频（I2V）。建议用 MiniMax-Hailuo-02，镜头稳定性最好。"), h("div", { className: "SPR_keyRow" }, h("label", { className: "SPR_field" }, h("span", { className: "SPR_fieldLabel" }, `API Key ${config.minimaxApiKeySet ? `（已配置 ${config.minimaxApiKeyHint}）` : "（未配置）"}`), h("input", {
                className: "SPR_input",
                type: "password",
            ...
            }), h("datalist", { id: "SPR_minimax_hosts" }, (config.minimaxHosts ?? []).map((host) => h("option", { key: host.id, value: host.id }, host.label))), h("span", { className: "SPR_fieldLabel" }, `当前协议：${minimaxCaps.protocol === "v2" ? `v2（${config.minimaxPathPrefix ?? ""}/v2/video_generation）` : `v1（${config.minimaxPathPrefix ?? ""}/v1/video_generation）`}`))), h("div", { className: "SPR_toolbar" }, h(BusyBtn, {
            ...
            }, "测试连接（只校验 Key，不产生费用）"))), h("div", { className: "SPR_settingsGroup" }, h("h3", null, "新建项目的默认参数"), h("p", null, "这些值会成为每个新项目的初始设置，之后可在项目里单独调整。"), h("div", { className: "SPR_fields" }, h(NumField, {
                label: "单格宽（px）",
