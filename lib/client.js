            ...
            return h("div", { className: "SPR_rigPanel" }, h("div", { className: "SPR_toolbar" }, h("span", { className: "SPR_refRow" }, "生图模型"), h("span", { className: "SPR_badge" }, globalConfig?.arkModel ?? job?.settings?.model ?? "未配置"), h("span", { className: "SPR_refRow" }, "（模型在「设置 → 游戏素材大师」里改）"), h("span", { className: "SPR_spacer" }), headerButtons), notice !== null
                ...
                : h(React.Fragment, null, h("div", { className: "SPR_toolbar", style: { flexWrap: "wrap" } }, h("span", { className: "SPR_refRow" }, "任务"), h("select", { className: "SPR_input", style: { width: 260 }, value: jobId ?? "", onChange: (event) => setJobId(event.target.value) }, jobs.map((entry) => h("option", { key: entry.id, value: entry.id }, `${entry.name}（部件 ${entry.partCount}）`))), job !== null ? h("span", { className: "SPR_badge" }, job.id) : null, stageBusy ? h(BusyBadge, { show: true, text: "本阶段进行中…" }) : null), job === null
                    ...
                    stage === "atlas"
                        ...
                        }, "打包纹理图集"), job.atlas?.text !== null && job.atlas?.text !== undefined ? h("a", { className: "SPR_link", href: job.atlas.text, target: "_blank", rel: "noreferrer" }, "查看 skeleton.atlas") : null, job.atlas?.image !== null && job.atlas?.image !== undefined ? h("a", { className: "SPR_link", href: job.atlas.image, download: "skeleton.png" }, "下载 skeleton.png") : null, job.atlas?.width !== undefined ? h(Chip, { kind: "ready", text: `${job.atlas.width}×${job.atlas.height} · ${job.atlas.regions} 区域` }) : null), job.atlas?.url !== null && job.atlas?.url !== undefined
                            ? h("div", { className: "SPR_rigAtlasWrap" }, h("img", { src: job.atlas.url, alt: "atlas" }))
                            : h("p", { className: "SPR_hint", style: { marginTop: 10 } }, job.rig?.status === "ready" ? "点「打包纹理图集」生成 skeleton.png + skeleton.atlas。" : "先完成第③步骨骼构建。"), h("div", { className: "SPR_toolbar", style: { marginTop: 12 } }, h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: true, stage: "atlas" }), "第④步已通过"), on: rigStageOf(job, "atlas").approved === true, disabled: job.atlas?.status !== "ready" }, rigStageOf(job, "atlas").approved === true ? "第④步：已通过" : "第④步：通过"), h(Btn, { onClick: () => void run(() => api.saveRigJob({ jobId: job.id, approved: false, stage: "atlas" })) }, "取消通过")), h("p", { className: "SPR_hint", style: { marginTop: 10 } }, "导入 Spine：把 skeleton.json、skeleton.atlas、skeleton.png 三个文件放在同一目录，打开 Spine 时选 skeleton.json 即可。"))
