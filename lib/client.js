...
`;
        ...
        function RigLayoutEditor({ job, selected, onSelect, onCommit, showReference }) {
            ...
            React.useEffect(() => {
                ...
                const up = () => {
                    ...
                    if (dx !== 0 || dy !== 0) {
                        ...
                        if (base !== undefined)
                            void onCommit(drag.name, { x: base.x + dx, y: base.y + dy });
                    }
                };
            ...
            if (compositeUrl === null)
                return h("p", { className: "SPR_hint" }, "还没有装配结果。");
            return h("div", null, showReference === true
                ? h("div", { className: "SPR_rigSideBySide" }, h("figure", null, h("figcaption", null, "参考图（角色整图）"), referenceUrl !== null ? h("img", { src: referenceUrl, alt: "reference" }) : null), h("figure", null, h("figcaption", null, "合成图（部件按当前摆放叠出来）"), h("img", { src: compositeUrl, alt: "composite" })))
                : null, h("div", { className: "SPR_rigCanvasWrap", "data-drag": drag !== null ? "true" : undefined, ref: wrapRef }, h("img", { src: compositeUrl, alt: "composite", onLoad: onImageLoad, draggable: false }), 
            // 先验框用虚线画出来：用户要能看出「agent 说的位置」和「实际摆的位置」差多少，
            // 这也决定了「要不要相信这个先验、要不要重新给」。
            Object.entries(hints).map(([name, box]) => h("div", {
                key: `hint:${name}`,
                ...
                if (item === undefined || item.matched !== true)
                    return null;
            ...
            })), h("p", { className: "SPR_hint" }, `画布 ${canvasW}×${canvasH}。点方块选中，拖动即微调位置（松手保存）；也可以在下面对话框里精确调数值。`, Object.keys(hints).length > 0 ? `虚线框是 agent 给的视觉先验（${Object.keys(hints).length} 个）。` : ""));
        }
        /**
         * 模块④的主体界面。
         *
            ...
            return h("div", { className: "SPR_rigPanel" }, h("div", { className: "SPR_toolbar" }, h("span", { className: "SPR_refRow" }, "生图模型"), h("span", { className: "SPR_badge" }, globalConfig?.arkModel ?? job?.settings?.model ?? "未配置"), h("span", { className: "SPR_refRow" }, "（模型在「设置 → 游戏素材大师」里改）"), h("span", { className: "SPR_spacer" }), headerButtons), notice !== null
                ...
                : h(React.Fragment, null, h("div", { className: "SPR_toolbar", style: { flexWrap: "wrap" } }, h("span", { className: "SPR_refRow" }, "任务"), h("select", { className: "SPR_input", style: { width: 260 }, value: jobId ?? "", onChange: (event) => setJobId(event.target.value) }, jobs.map((entry) => h("option", { key: entry.id, value: entry.id }, `${entry.name}（部件 ${entry.partCount}）`))), job !== null ? h("span", { className: "SPR_badge" }, job.id) : null, stageBusy ? h(BusyBadge, { show: true, text: "本阶段进行中…" }) : null), job === null
                    ...
                    stage === "layout"
                        ? h("div", { className: "SPR_rigStage" }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, "② 装配定位"), h("span", { className: "SPR_rigStageHint" }, "多尺度模板匹配把每个部件摆回参考姿态。**本地计算，免费**：失败或摆错只重跑那几个部件，不用整批重来。"), h(StatusChip, { node: rigStageOf(job, "layout") })), h("div", { className: "SPR_rigEditorRow" }, h(BusyBtn, {
                        ...
                        }, `只重试未命中的 ${(job.review?.unmatched ?? []).length} 个`), h(Btn, { onClick: () => setShowReference((current) => !current), on: showReference }, showReference ? "只显示合成图" : "并排显示参考图")), h(RigLayoutEditor, {
                            job,
                            selected,
                            showReference,
                            ...
                            ? h("div", { className: "SPR_rigStage", style: { marginTop: 10 } }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, `微调：${selected}`), h("span", { className: "SPR_rigStageHint" }, "改完立刻重出合成图；这一步只影响这一个部件。")), h("div", { className: "SPR_rigEditorRow" }, h(NumField, { label: "x", value: selectedItem.x, onChange: (value) => void run(() => api.saveRigLayoutItem({ jobId: job.id, name: selected, x: value })) }), h(NumField, { label: "y", value: selectedItem.y, onChange: (value) => void run(() => api.saveRigLayoutItem({ jobId: job.id, name: selected, y: value })) }), h(NumField, { label: "宽", value: selectedItem.width, min: 1, onChange: (value) => void run(() => api.saveRigLayoutItem({ jobId: job.id, name: selected, width: value })) }), h(NumField, { label: "高", value: selectedItem.height, min: 1, onChange: (value) => void run(() => api.saveRigLayoutItem({ jobId: job.id, name: selected, height: value })) }), h(NumField, { label: "层级 z", value: selectedItem.z, onChange: (value) => void run(() => api.saveRigLayoutItem({ jobId: job.id, name: selected, z: value })) }), h(NumField, { label: "旋转", value: selectedItem.rotation, step: 5, onChange: (value) => void run(() => api.saveRigLayoutItem({ jobId: job.id, name: selected, rotation: value })) }), h(BusyBtn, {
                            ...
                            : null, (job.review?.unmatched ?? []).length > 0
