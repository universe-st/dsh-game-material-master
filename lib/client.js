...
`;
        ...
        function renderImageStage(ctx) {
            ...
            }, "重置提示词为默认"), h(BusyBadge, { show: batch, text: batchLabel }), project.source === null ? h("span", { className: "SPR_refRow" }, "请先上传源图") : null), h("div", { className: "SPR_grid" }, DIRECTIONS.map((direction) => {
        ...
        function renderFrameStage(ctx) {
            ...
            return h(React.Fragment, null, h("div", { className: "SPR_fields" }, h(NumField, {
                ...
                return h("div", { key: direction.key, className: "SPR_node", "data-busy": nodeBusy || waiting ? "true" : undefined }, h("div", { className: "SPR_nodeTop" }, h("span", { className: "SPR_nodeTitle" }, direction.label), nodeBusy ? h(Chip, { kind: "running", text: "抽帧中" }) : h(StatusChip, { node })), node?.strip !== undefined
                    ? h(MediaBox, { overlay: nodeBusy || waiting, text: overlayText }, h("img", { className: "SPR_thumb", src: assetUrl(project, node.strip, node.updatedAt), alt: `${direction.label} 序列帧` }))
                    : h("div", { className: "SPR_thumbEmpty" }, nodeBusy || waiting ? "正在抽帧…" : "尚未抽帧"), nodeBusy || waiting ? h(BusyBadge, { show: true, text: overlayText }) : null, node?.duration !== undefined
