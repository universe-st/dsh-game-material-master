                    ...
                    : h("div", { className: "SPR_tlWrap", "data-testid": "rig-tl-track" }, h("div", { className: "SPR_tlRuler" }, [0, 0.25, 0.5, 0.75, 1].map((ratio) => h("span", { key: ratio, className: "SPR_tlTick", style: { left: `${ratio * 100}%` } }, `${(ratio * draft.duration).toFixed(2)}s`))), tracks.map((track, trackIndex) => h("div", { key: `${track.bone}/${track.kind}`, className: "SPR_tlRow" }, h("span", { className: "SPR_tlName", title: `${track.bone} · ${TRACK_LABELS[track.kind]}` }, `${track.bone}`, h("em", { className: "SPR_tlKind" }, TRACK_LABELS[track.kind])), h("div", {
                        ...
                        return h("button", {
                            ...
                            onMouseDown: (event) => {
                                ...
                                if (index > 0)
                                    dragRef.current = { bone: track.bone, kind: track.kind, index };
                            }
                        });
                    })), h("button", {
                        type: "button",
                ...
                : h(React.Fragment, null, h("div", { className: "SPR_toolbar", style: { flexWrap: "wrap" } }, h("span", { className: "SPR_refRow" }, "任务"), h("select", { className: "SPR_input", style: { width: 260 }, value: jobId ?? "", onChange: (event) => setJobId(event.target.value) }, jobs.map((entry) => h("option", { key: entry.id, value: entry.id }, `${entry.name}（部件 ${entry.partCount}）`))), job !== null ? h("span", { className: "SPR_badge" }, job.id) : null, stageBusy ? h(BusyBadge, { show: true, text: "本阶段进行中…" }) : null), job === null
                    ...
                    stage === "rig"
                        ? h("div", { className: "SPR_rigStage" }, h("div", { className: "SPR_rigStageHead" }, h("span", { className: "SPR_rigStageTitle" }, "③ 骨骼与动画"), h("span", { className: "SPR_rigStageHint" }, "按部件语义自动推骨骼层级，并生成六个动画预设。**本地计算，免费**。下面直接播放验收。"), h(StatusChip, { node: rigStageOf(job, "rig") })), h("div", { className: "SPR_rigEditorRow" }, h(BusyBtn, {
                            ...
                            : h("p", { className: "SPR_hint", style: { marginTop: 10 } }, job.layout?.status === "ready" ? "点「生成骨骼与动画」得到 skeleton.json 与可播放预览。" : "先完成第②步装配定位。"), h(RigAnimationPanel, { job, api, run, busy, activeKey: K_RIG_BONES }), h(RigTimelineEditor, { job, api, run, busy, activeKey: K_RIG_BONES, onRebuild: true }), h(RigBoneEditor, { job, api, run, busy, activeKey: K_RIG_BONES }), 
