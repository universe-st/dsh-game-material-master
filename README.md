...
- **拆件质量由生图模型决定，而且这是目前最大的瓶颈**。提示词已经明确要求
  ...
  插件会自己把这个判断讲清楚：整批部件平均相似度低于 0.45 时直接告警「拆件图里的
  角色很可能与参考图不是同一个」，并给出三条可走的路——
  ① 用「部件 PNG」上传框传自己的部件（**文件名即部件名**，完全绕开生图）；
  ② 换更强的生图模型（如 Seedream 5.0 Pro）重新拆件；
  ③ 在合成图上手工拖到正确位置。
- **自动装配的精度取决于美术细节**。定位用的是灰度相关系数（叠加了色相一致度），
  对**平涂、低细节**的角色会明显吃力：同色的大片区域内部没有区分度，左右肢之间
  几乎不可分辨。插件用「镜像部件必须分居中线两侧」的几何约束兜底，并对相似度
  低于 0.5 的部件明确标注，不会假装自己摆对了。
