...
export function solveLayout(reference: Rgba, parts: LayoutPartInput[], options: LayoutSolveOptions = {}): LayoutSolveResult {
  ...
  for (const [name, box] of ranked) {
    ...
    const second = matchPartInPyramid(
      buildReferencePyramid(blanked, longEdge, foregroundTolerance),
      part.rgba,
      { scales: narrow, evidenceWeight, minCoverage: Math.max(0.4, minCoverage - 0.1) }
    );
