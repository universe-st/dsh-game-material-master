...
export interface SequenceVideoState {
  status: NodeStatus;
  taskId?: string;
  remoteStatus?: string;
  file?: string;
...
export interface SequenceSheetState {
  status: NodeStatus;
  file?: string;
  width?: number;
  height?: number;
...
export interface SequenceJob {
  ...
  suffix: string;
  refs: {
    firstFrame?: SequenceRef;
    lastFrame?: SequenceRef;
    referenceImages: SequenceRef[];
    referenceVideos: SequenceRef[];
  };
  settings: {
    model: string;
    duration: number;
...
export interface SequenceJobSummary {
  id: string;
  name: string;
  createdAt: number;
  updatedAt: number;
...
export function sequenceJobDir(id: string): string {
...
export function sequenceAssetPath(id: string, relative: string): string {
...
export function isValidSequenceJobId(id: string): boolean {
  return JOB_ID_PATTERN.test(id);
}

export async function ensureSequenceJobLayout(id: string): Promise<void> {
  for (const sub of ["refs", "video-refs", "videos", "frames", "keyed", "out"]) {
    await mkdir(join(sequenceJobDir(id), sub), { recursive: true });
  }
}

export async function createSequenceJob(name: string): Promise<SequenceJob> {
...
export async function readSequenceJob(id: string): Promise<SequenceJob | undefined> {
  if (!isValidSequenceJobId(id)) return undefined;
  const raw = await readJson<SequenceJob>(jobFile(id));
  if (raw === undefined) return undefined;
  return normalizeSequenceJob(raw);
}

export async function writeSequenceJob(job: SequenceJob): Promise<void> {
  job.updatedAt = Date.now();
  await writeJsonAtomic(jobFile(job.id), job);
}

export async function listSequenceJobs(): Promise<SequenceJobSummary[]> {
...
export function summarizeSequenceJob(job: SequenceJob): SequenceJobSummary {
  return {
    id: job.id,
    name: job.name,
...
export async function deleteSequenceJob(id: string): Promise<void> {
  stopSequencePoller(id);
  await rm(sequenceJobDir(id), { recursive: true, force: true });
}

export async function sequenceJobExists(id: string): Promise<boolean> {
  try {
...
function normalizeSequenceJob(raw: any): SequenceJob {
  const ref = (value: any): SequenceRef | undefined =>
    typeof value?.file === "string" ? { file: value.file, name: String(value.name ?? basename(value.file)) } : undefined;
  const refList = (value: any): SequenceRef[] =>
    Array.isArray(value) ? value.map(ref).filter((r): r is SequenceRef => r !== undefined) : [];
...
export function buildSequencePrompt(job: SequenceJob): { prompt: string } | { error: string } {
  const base = job.prompt.trim();
  if (base === "") return { error: "提示词为空，请先填写视频提示词" };
...
const jobsRunning = new Map<string, Set<string>>();
...
export function listSequenceTasks(jobId: string): string[] {
  return [...(jobsRunning.get(jobId) ?? [])];
}

function kick(jobId: string, taskKey: string, fn: () => Promise<void>): boolean {
  if (disposed) return false;
  const set = jobsRunning.get(jobId) ?? new Set<string>();
  ...
  void (async () => {
    try {
      ...
      set.delete(taskKey);
      if (set.size === 0) jobsRunning.delete(jobId);
    }
  })();
  return true;
}
...
function assertIdle(jobId: string, taskKey: string, what: string): void {
  if (jobsRunning.get(jobId)?.has(taskKey) === true) throw new Error(`${what}已在进行中，请等它跑完`);
}
...
async function toDataUriIfPresent(jobId: string, file: string | undefined): Promise<string | undefined> {
...
async function videoDataUri(jobId: string, file: string): Promise<string | undefined> {
  const absolute = sequenceAssetPath(jobId, file);
  const size = await fileSize(absolute);
  // 请求体总上限 64MB；base64 会放大 1/3，所以原始文件卡在 40MB。
  if (size > 40 * 1024 * 1024) throw new Error(`参考视频 ${basename(file)} 超过 40 MB，平台要求改用公网 URL，本地文件请先压缩`);
  const bytes = await readFile(absolute);
  const mime = file.toLowerCase().endsWith(".mov") ? "video/quicktime" : "video/mp4";
  return `data:${mime};base64,${bytes.toString("base64")}`;
}

export function ensureSequencePoller(jobId: string): void {
...
export function stopSequencePoller(jobId: string): void {
  const timer = pollers.get(jobId);
  if (timer !== undefined) clearInterval(timer);
  pollers.delete(jobId);
}

export async function pollSequenceOnce(jobId: string): Promise<void> {
  if (disposed) return;
  const job = await readSequenceJob(jobId);
  if (job === undefined) {
...
export function startSequenceFrames(jobId: string, count?: number): { started: boolean; reason?: string } {
  try {
...
export async function extractSequenceFrames(jobId: string, count?: number): Promise<void> {
  const job = await readSequenceJob(jobId);
  if (job === undefined) throw new Error(`任务不存在：${jobId}`);
  ...
  try {
    ...
    appendJobLog(
      fresh.log,
      "info",
      `抽帧完成：${files.length} 张 / ${extracted.width}×${extracted.height} / 视频时长 ${extracted.duration.toFixed(2)} 秒`
    );
    await writeSequenceJob(fresh);
...
export function startSequenceKey(jobId: string): { started: boolean; reason?: string } {
  try {
...
export async function keySequenceFrames(jobId: string): Promise<void> {
  const job = await readSequenceJob(jobId);
  if (job === undefined) throw new Error(`任务不存在：${jobId}`);
...
/** 把所有抠好的帧横向拼成一张条图。 */
export async function composeSequenceSheet(jobId: string): Promise<void> {
  const job = await readSequenceJob(jobId);
  if (job === undefined) throw new Error(`任务不存在：${jobId}`);
  if (job.frames.raw === undefined || job.frames.width === undefined || job.frames.height === undefined) {
    throw new Error("还没有抽过帧，请先抽帧");
  ...
  const row: SheetRow = { key: "sequence", frames, width, height };
...
export type SequenceRefKind = "firstFrame" | "lastFrame" | "referenceImage" | "referenceVideo";

export async function uploadSequenceRef(jobId: string, kind: SequenceRefKind, name: string, base64: string): Promise<void> {
  const job = await readSequenceJob(jobId);
  if (job === undefined) throw new Error(`任务不存在：${jobId}`);
  const clean = base64.replace(/^data:[^;]+;base64,/, "");
  const bytes = Buffer.from(clean, "base64");
  if (bytes.length === 0) throw new Error("文件数据为空");

  if (kind === "referenceVideo") {
    if (bytes.length > 40 * 1024 * 1024) throw new Error("参考视频超过 40 MB，平台要求改用公网 URL");
    if (job.refs.referenceVideos.length >= 3) throw new Error("参考视频最多 3 段");
  ...
  await writeFile(sequenceAssetPath(jobId, relative), bytes);
  const entry: SequenceRef = { file: relative, name: safeName(name) };

  if (kind === "firstFrame") job.refs.firstFrame = entry;
  else if (kind === "lastFrame") job.refs.lastFrame = entry;
  else {
    if (job.refs.referenceImages.length >= 9) throw new Error("参考图最多 9 张");
...
export async function removeSequenceRef(jobId: string, kind: SequenceRefKind, file?: string): Promise<void> {
  const job = await readSequenceJob(jobId);
  if (job === undefined) throw new Error(`任务不存在：${jobId}`);
  if (kind === "firstFrame") job.refs.firstFrame = undefined;
  else if (kind === "lastFrame") job.refs.lastFrame = undefined;
  else if (kind === "referenceImage") job.refs.referenceImages = job.refs.referenceImages.filter((r) => r.file !== file);
  else job.refs.referenceVideos = job.refs.referenceVideos.filter((r) => r.file !== file);
  await writeSequenceJob(job);
}
