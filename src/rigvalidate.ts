...
export function validateSpineWire(spine: any): ValidationReport {
  ...
  {
    const boneNames = new Set<string>((Array.isArray(spine?.bones) ? spine.bones : []).map((bone: any) => bone?.name).filter((name: unknown) => typeof name === "string"));
    const slotNames = new Set<string>((Array.isArray(spine?.slots) ? spine.slots : []).map((slot: any) => slot?.name).filter((name: unknown) => typeof name === "string"));
    const attachmentOf = (slotName: string, attName: string): any => {
...
export function validateSkeletonAtlasMatch(spine: any, pages: AtlasPageLike[]): ValidationReport {
  ...
  for (const [slotName, byName] of Object.entries<any>(attachments)) {
    ...
    for (const attachmentName of Object.keys(byName)) {
      ...
      if (byName[attachmentName]?.type === "path") continue;
