...
export interface RigMeshBoneInfluence {
  /** 在 `bones` 数组里的下标。 */
  bone: number;
  weight: number;
}
...
export interface RigMesh {
  cols: number;
  rows: number;
  /** 顶点局部坐标（部件左上角为原点，Y 向下）：[x0,y0, x1,y1, …]。 */
  vertices: number[];
  /** 归一化 UV：[u0,v0, u1,v1, …]。 */
  uvs: number[];
  /** 三角形索引（每 3 个一组）。 */
  triangles: number[];
  /** 参与蒙皮的骨骼名（与 `slotPose` / `bonePose` 的下标对应）。 */
  bones: string[];
  /** 逐顶点的骨骼影响，顺序与 `vertices` 的顶点一一对应。 */
  weights: RigMeshBoneInfluence[][];
}

export interface RigMeshBone {
  name: string;
  /** 骨骼原点的世界坐标（参考图像素，Y 向下）。 */
  x: number;
  y: number;
}
...
function clampDivisions(value: unknown, fallback: number): number {
...
export function buildRigMesh(options: {
  ...
  rows?: number;
  bones: RigMeshBone[];
  /** 部件在画布上的左上角（用来把顶点换算到世界坐标算权重）。 */
  x: number;
  y: number;
  maxInfluences?: number;
  /** 自动权重的衰减指数：越大越"硬"（越贴近最近的那根骨骼）。 */
  falloff?: number;
}): RigMesh {
  ...
  const uvs: number[] = [];
  const weights: RigMeshBoneInfluence[][] = [];
...
/** 顶点个数。 */
export function meshVertexCount(mesh: RigMesh): number {
 ...
 */
export function sampleDeform(mesh: RigMesh, offsets: number[] | undefined, localX: number, localY: number, width: number, height: number): { x: number; y: number } {
  if (offsets === undefined || offsets.length !== mesh.vertices.length) return { x: localX, y: localY };
...
/**
 * 生成一段"飘动"的 FFD 位移：越靠近**远端**（远离固定端的那一侧）位移越大。
 *
 * 用正弦而不是随机：循环动画的首尾必须闭合（与动画预设同一个要求），
 * 而且「摆动」这件事本来就该是周期函数。
 *
 * ⚠️ **相位只随时间推进，不随高度变化**。第一版写成 `sin(along·cycles + phase)`，
 * 于是同一时刻不同高度的位移方向不同——相邻行被反向拉扯，整条裙子会**横向撑开**
 * 变成一个气球，而不是左右摆。现在 `along` 只作为**权重**（上缘 0、下缘 1），
 * `cycles` 变成「一个循环里摆几次」。
 */
export function buildWaveDeform(options: {
  ...
  width: number;
  height: number;
  /** 相位（0~1）：同一条轨迹上的不同骨骼错开一点，看起来才像布料在飘。 */
  phase?: number;
  amplitude?: number;
  /** 位移方向（弧度）：0 = 向 +x。 */
  direction?: number;
  /** 停滞的一侧：`"top"` 表示上缘不动、下缘甩出去（裙摆就是这个形态）。 */
  anchor?: "top" | "bottom" | "none";
  /** 波形周期数。 */
  cycles?: number;
}): { offsets: number[]; maxDisplacement: number } {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // **相位只跟时间有关**（见函数头的注释）：沿 y 变化的是权重，不是相位。
    const sway = Math.sin((phase * cycles) * Math.PI * 2);
