...
const ROLE_SET = new Set<string>(RIG_ROLES);

export function isRigRole(value: unknown): value is RigRole {
 ...
 */
export const ROLE_NOUNS: Record<RigRole, string> = {
  head: "head",
  neck: "neck",
  torso: "torso",
  hip: "hip / pelvis",
 ...
 */
export const ROLE_ANCHORS: Record<RigRole, { proximal: [number, number]; distal: [number, number] }> = {
 ...
 */
export const ROLE_PARENT_ROLE: Record<RigRole, RigRole | undefined> = {
  head: "neck",
  neck: "torso",
  torso: "hip",
  hip: undefined,
...
export interface PartSemantics {
  ...
  /** 自由标签，供 AI 按组批量操作（改色/重绘），例如 "cloth"、"metal"。 */
  tags: string[];
  /** 由谁给出：`default` = 表兜底、`ai` = 模型提案、`human` = 人改过。 */
  source?: "default" | "ai" | "human";
}
...
function normalizePair(value: unknown, fallback: [number, number]): [number, number] {
  if (!isFinitePair(value)) return [fallback[0], fallback[1]];
 ...
 * 「上臂」的规则抢走。
 */
const ROLE_KEYWORDS: Array<[RigRole, string[]]> = [
...
export function roleOfName(name: string): RigRole {
  const lower = name.toLowerCase();
  for (const [role, keys] of ROLE_KEYWORDS) {
    if (keys.some((key) => lower.includes(key))) return role;
  }
  return "accessory";
}
 ...
 */
export function parentNameOf(name: string, roleByName: Map<string, RigRole>): string | undefined {
  ...
  while (parentRole !== undefined && !visited.has(parentRole)) {
    ...
    if (candidates.length > 0) {
      ...
      if (side !== "") {
        ...
        if (sameSide !== undefined) return sameSide[0];
      }
...
export function sideOfName(name: string): "left" | "right" | "" {
  const lower = name.toLowerCase();
  if (lower.includes("left") || name.includes("左")) return "left";
  if (lower.includes("right") || name.includes("右")) return "right";
  return "";
}
 ...
 */
export function defaultSemanticsOf(names: string[]): PartSemantics[] {
  const roleByName = new Map<string, RigRole>();
  for (const name of names) roleByName.set(name, roleOfName(name));
  ...
  return names.map((name) => {
    ...
    const side = sideOfName(name);
    if (side !== "") tags.add(side);
    return {
      name,
      role,
      parent: parentNameOf(name, roleByName),
      proximal: [anchors.proximal[0], anchors.proximal[1]] as [number, number],
      distal: [anchors.distal[0], anchors.distal[1]] as [number, number],
      tags: [...tags],
...
export interface SemanticsIssue {
  /** 出问题的部件名；整体性问题为 `""`。 */
  name: string;
  level: "error" | "warning";
  message: string;
...
export interface SemanticsValidation {
  ...
  /** 归一化后的语义表——总是可用，即使有 error（UI 要能显示「哪里不对」）。 */
  parts: PartSemantics[];
}
...
export function validateSemantics(
  input: Array<Partial<PartSemantics> & { name: string }>,
  options: { humanoid?: boolean } = {}
): SemanticsValidation {
  const errors: SemanticsIssue[] = [];
  const warnings: SemanticsIssue[] = [];

  // ① 名字唯一且非空。
  const seen = new Set<string>();
  const unique: Array<Partial<PartSemantics> & { name: string }> = [];
  ...
  // ② 角色与锚点。
  const parts: PartSemantics[] = unique.map((raw) => {
    let role: RigRole;
  ...
  if (humanoid) {
    const has = (role: RigRole) => parts.some((part) => part.role === role);
    if (!has("head")) warnings.push({ name: "", level: "warning", message: "判定为人形，但没有角色为「头部」的部件" });
    if (!has("torso") && !has("hip")) warnings.push({ name: "", level: "warning", message: "判定为人形，但没有躯干或胯部，骨骼会以 root 为中心" });
    ...
    // **肢体**出现在头的上方。
    const LIMB_ROLES: RigRole[] = ["upperArm", "lowerArm", "hand", "upperLeg", "lowerLeg", "foot"];
    ...
    for (const role of ["upperArm", "lowerArm", "hand", "upperLeg", "lowerLeg", "foot"] as RigRole[]) {
      const sides = new Set(parts.filter((part) => part.role === role).map((part) => sideOfName(part.name)));
      if (sides.has("left") && !sides.has("right")) warnings.push({ name: "", level: "warning", message: `有左侧${ROLE_LABELS[role]}但没有右侧` });
      if (sides.has("right") && !sides.has("left")) warnings.push({ name: "", level: "warning", message: `有右侧${ROLE_LABELS[role]}但没有左侧` });
    }
  }
...
export function mergeSemantics(
  current: Array<Partial<PartSemantics> & { name: string }>,
  patches: Array<{ name: string } & Partial<Omit<PartSemantics, "name">>>
): Array<Partial<PartSemantics> & { name: string }> {
