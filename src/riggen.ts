 ...
 * 存下来之后，界面、对话工具、装配阶段都能直接引用同一份结论。
 */
export interface RigQaState {
  ok: boolean;
  /** 0~1 可信度，只用来给直觉；判断一律以 `issues` 为准。 */
  score: number;
  summary: string;
  /** 参与检查的部件数（不含隐藏）。 */
  checked: number;
  duplicates: Array<{ parts: string[]; minOverlap: number; mirroredOverlap: number; symmetric: boolean }>;
  issues: QaIssue[];
  /** 参考图里大致有几个独立部位（由人/agent 看图给，给不出就不做这项判断）。 */
...
export interface RigJob {
  ...
  /**
   * **手工编辑过的动画**：id → 一份完整的简写动画（时间轴数据）。
   *
   * 六个预设是「参数表 + 实例化器」，`animationSettings` 只给两个旋钮（时长 / 幅度）。
   * 一旦有人真的去拖关键帧，参数化就不够了——这时把那份简写**整份**存下来，
   * 之后这台动画就以它为准，重跑骨骼不会把它覆盖回去（与 `boneOffsets` 同一个道理：
   * 自动推导是可再生的，人的改动不是）。
   *
   * 简写 = `{bones: {骨骼名: {rotate|translate|scale: [{time, angle|x|y, curve}]}}}`，
   * 与 `spine.ts` 的 `buildAnimationShorthands` 同构，所以导出前不需要任何转换，
...
/** 一份手工动画：简写时间轴 + 它自己的循环时长与是否循环。 */
export interface RigCustomAnimation {
  /** 循环时长（秒）。 */
  duration: number;
  loop: boolean;
  /** 骨骼名 → 轨道类型 → 关键帧数组。 */
  bones: Record<string, Record<string, RigKeyframe[]>>;
  /** 这份动画是「从预设实例化后改的」还是「AI 提议的」，只用于界面提示。 */
  source?: "preset" | "ai" | "human";
  updatedAt?: number;
}

/** 一个关键帧。`curve` 挂在**段终点**那一帧上（见 `spine.ts` 的 `convertTimeline`）。 */
export interface RigKeyframe {
  time: number;
  angle?: number;
  x?: number;
  y?: number;
  curve?: number[] | "stepped" | null;
}
...
function normalizeRigJob(raw: any): RigJob {
  ...
  return {
    ...
    animations: (() => {
      ...
      const out: Record<string, RigCustomAnimation> = {};
      for (const [id, value] of Object.entries<any>(input)) {
        if (!isAnimationId(id)) continue;
...
function normalizeQaState(raw: any): RigQaState | undefined {
  ...
  return {
    ...
    duplicates: Array.isArray(raw.duplicates)
      ? raw.duplicates
          .filter((group: any) => Array.isArray(group?.parts))
          .map((group: any) => ({
...
function invalidateFrom(job: RigJob, stage: AnyStage): void {
  ...
  const at = (key: AnyStage) => STAGE_ORDER.indexOf(key);
  if (from <= at("sheet")) {
  ...
  if (from < at("layout")) job.layout = { status: "empty", items: {} };
  else job.layout.approved = false;
  if (from <= at("rig")) job.rig = { status: "empty" };
  if (from <= at("atlas")) job.atlas = { status: "empty" };
}
 ...
 */
function normalizeCustomAnimation(
  raw: unknown,
  fallback: { duration: number; loop: boolean },
  issues: string[]
): RigCustomAnimation | undefined {
  ...
  const bones: RigCustomAnimation["bones"] = {};
  let tracks = 0;
  for (const [boneName, kindsRaw] of Object.entries<any>(input.bones)) {
    if (typeof boneName !== "string" || boneName === "") continue;
    if (kindsRaw === null || typeof kindsRaw !== "object") continue;
    const kinds: Record<string, RigKeyframe[]> = {};
    for (const [kind, framesRaw] of Object.entries<any>(kindsRaw)) {
      if (kind !== "rotate" && kind !== "translate" && kind !== "scale") continue;
      if (!Array.isArray(framesRaw)) continue;
      ...
      for (const frameRaw of framesRaw.slice(0, MAX_KEYFRAMES_PER_TRACK)) {
        ...
        } else if (curve === null || curve === undefined) frame.curve = null;
        else {
    ...
    if (Object.keys(kinds).length > 0) bones[boneName] = kinds;
  }
 ...
 */
export async function saveRigAnimation(
  jobId: string,
  input: { id: string; animation?: unknown; boneNames?: string[] }
): Promise<{ id: string; tracks: number; keyframes: number; issues: string[] }> {
  const job = await readRigJob(jobId);
  if (job === undefined) throw new Error(`任务不存在：${jobId}`);
  const id = typeof input.id === "string" ? input.id : "";
  const preset = animationPresetOf(id);
  if (preset === undefined) throw new Error(`没有这台动画：${id}`);
  ...
  if (base === undefined) throw new Error(`这台动画在当前部件集下没有任何轨道（预设依赖的骨骼都不存在）：${id}`);
  ...
  appendJobLog(
    job.log,
    "info",
    `${source === undefined ? "开始编辑" : "已更新"}动画「${preset.label}」：${tracks} 条轨道 / ${frames} 个关键帧`
  );
  for (const issue of issues.slice(0, 6)) appendJobLog(job.log, "warn", `动画「${preset.label}」：${issue}`);
  await writeRigJob(job);
 ...
 */
export async function getRigAnimation(
  jobId: string,
  id: string
): Promise<{ id: string; label: string; custom: boolean; duration: number; loop: boolean; bones: Record<string, Record<string, RigKeyframe[]>>; boneOrder: string[] }> {
  const job = await readRigJob(jobId);
  if (job === undefined) throw new Error(`任务不存在：${jobId}`);
  const preset = animationPresetOf(id);
  if (preset === undefined) throw new Error(`没有这台动画：${id}`);
  ...
  if (draft === undefined) throw new Error(`这台动画在当前部件集下没有任何轨道：${id}`);
  // 骨骼顺序按骨架层级给，界面上的轨道列表才跟骨骼编辑器对得上。
  const order = boneOrderOf(job);
  const bones: Record<string, Record<string, RigKeyframe[]>> = {};
  for (const name of order) if (draft.bones[name] !== undefined) bones[name] = draft.bones[name];
  for (const [name, kinds] of Object.entries(draft.bones)) if (bones[name] === undefined) bones[name] = kinds;
  return {
    id,
    label: preset.label,
 ...
 */
function boneOrderOf(job: RigJob): string[] {
  const names = boneNamesOf(job);
  const known = new Set(names);
  const seen = new Set<string>();
  const out: string[] = [];
  for (const bone of job.rig.boneList ?? []) {
    if (bone.name === "root" || !known.has(bone.name) || seen.has(bone.name)) continue;
    seen.add(bone.name);
    out.push(bone.name);
  }
...
export async function resetRigAnimation(jobId: string, id: string): Promise<{ touched: number }> {
  const job = await readRigJob(jobId);
  if (job === undefined) throw new Error(`任务不存在：${jobId}`);
  const animations = { ...(job.animations ?? {}) };
  if (animations[id] === undefined) return { touched: 0 };
...
function buildQaState(report: QaReport, expectedParts?: number): RigQaState {
  return {
    ok: report.ok,
    score: report.score,
 ...
 * 宁可不报，也不要瞎报。
 */
export async function runRigQa(jobId: string, expectedParts?: number): Promise<RigQaState> {
  const job = await readRigJob(jobId);
  if (job === undefined) throw new Error(`任务不存在：${jobId}`);
  const parts: QaPart[] = [];
  for (const part of job.parts) {
    if (part.status !== "ready" || part.file === undefined) continue;
    if (part.hidden === true) continue;
    try {
  ...
  if (expectedParts !== undefined) job.qa.expectedParts = expectedParts;
...
export async function solveRigLayout(jobId: string, names?: string[]): Promise<void> {
  ...
  try {
    ...
    if (fresh.qa.expectedParts !== undefined) fresh.qa.expectedParts = fresh.qa.expectedParts;
    await writeRigJob(fresh);
...
export async function buildRigOutput(jobId: string): Promise<void> {
  ...
  try {
    ...
    const loopReport = validateAnimationLoops(
      spine,
