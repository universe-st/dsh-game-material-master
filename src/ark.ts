...
function isFiveSeries(model: string): boolean {
...
export async function generateImage(request: ArkRequest, signal?: AbortSignal): Promise<ArkResult> {
  ...
  if (request.images.length === 1) body.image = request.images[0];
  else if (request.images.length > 1) body.image = request.images;
  // output_format 只有 5.0 系列接受；4.x 传了会 400。
  if (isFiveSeries(request.model)) body.output_format = "png";
  ...
  try {
    ...
    if (timeout.aborted) throw new ArkError(`生图请求超时（${Math.round(request.timeoutMs / 1000)} 秒）`);
    if (signal?.aborted === true) throw new ArkError("生图请求已被取消");
  ...
  if (typeof item.url === "string" && item.url !== "") {
    const download = await fetch(item.url, { signal: composed }).catch((error: unknown) => {
 ...
 */
export async function testArk(
  request: Omit<ArkRequest, "prompt" | "images" | "size" | "watermark">
): Promise<{ ok: true; model: string; bytes: number; ext: string }> {
