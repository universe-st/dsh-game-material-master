...
export interface DirectionDef {
  ...
  /** 界面上的中文名。 */
  label: string;
  /** 一句话朝向，插进生图提示词的 {facing}。只描述转身，不描述视线方向。 */
  facing: string;
  /** 脸部 / 身体可见部位，插进生图提示词的 {visibility}。 */
  visibility: string;
  /** 参考图来源：`source`（用户源图）或其它方向的 key。 */
  refs: string[];
}
...
export const DIRECTION_KEYS: string[] = DIRECTIONS.map((d) => d.key);

export function directionOf(key: string): DirectionDef | undefined {
  return DIRECTIONS.find((d) => d.key === key);
}

/** 罗盘顺时针顺序，用于整图行序。 */
export const COMPASS_ORDER: Compass[] = ["N", "NE", "E", "SE", "S", "SW", "W", "NW"];

/** 默认整图行序：北 → 东北 → 东 → 东南 → 南 → 西南 → 西 → 西北（罗盘顺时针）。 */
export const DEFAULT_ROW_ORDER: string[] = COMPASS_ORDER.map(
  (compass) => DIRECTIONS.find((direction) => direction.compass === compass)?.key ?? ""
).filter((key) => key !== "");
...
/**
 * 转圈模式下的八个截帧位置（时间轴上的先后顺序）。
 *
 * `ccw` 是默认值，依据是**实测**：MiniMax-H3 拿到「原地匀速转一整圈」的提示词后，
 * 角色鼻尖先转向画面右侧（俯视看是逆时针）——32 张候选帧里第 8 帧是右侧脸、
 * 第 24 帧是左侧脸。换模型或换提示词后如果反了，界面上点一下「转圈方向」即可，
 * 这只是**默认位置**的排法，用户拖过的圆圈一概以拖过的为准。
 */
 ...
 */
export const TURN_ORDER_CW: string[] = ["front", "downLeft", "left", "upLeft", "back", "upRight", "right", "downRight"];
export const TURN_ORDER_CCW: string[] = ["front", "downRight", "right", "upRight", "back", "upLeft", "left", "downLeft"];

/** 默认转圈方向（见 TurnDirection 的注释）。 */
export const TURN_DIRECTION_DEFAULT: TurnDirection = "ccw";

export function turnOrder(direction: TurnDirection): string[] {
 ...
 */
export function defaultTurnPicks(frameCount: number, direction: TurnDirection = TURN_DIRECTION_DEFAULT): Record<string, number> {
...
/** 把模板里的两个占位符换成具体方向的描述。 */
export function fillImagePrompt(template: string, facing: string, visibility: string): string {
  return template.replace(/\{facing\}/g, facing).replace(/\{visibility\}/g, visibility);
}

/** 把八方向默认提示词装配成一张表（每个方向一份，可单独编辑）。 */
export function defaultImagePrompts(): Record<string, string> {
